...
// 2008.3.12 cheon ill zea - 계급경험치 변화량 표시때문
...
//Test - Skill Cool Time Check Test함수

//SkillType으로 .. 동일한 스킬이 연속 사용될 때, 사용시간 차이를 SkillDelay.log에 출력해 준다.
void TestCheckSkillCooltime (int SkillType, int WeaponSpeed)
{
	...
	{
		ACTIONINFO_PACKET	packetType		= (*g_pActionInfoTable)[SkillType].GetPacketType();		//어떤 방식의 스킬인가?
		char strSkillInfo [256];
		switch (packetType)
		{
		case ACTIONINFO_PACKET_OTHER:
			wsprintf (strSkillInfo, "타인에게 사용하는 스킬");
			break;
		case ACTIONINFO_PACKET_SELF:
			wsprintf (strSkillInfo, "자신에게 사용하는 스킬");
			break;
		case ACTIONINFO_PACKET_ZONE:
			wsprintf (strSkillInfo, "타일에게 사용하는 스킬");
			break;
		default:
			wsprintf (strSkillInfo, "무얼까? %d", packetType);
			break;
		}
		const char *pSkillName = (*g_pActionInfoTable)[SkillType].GetName ();						//스킬 이름
		int	StartFrame = (*g_pActionInfoTable)[SkillType].GetStartFrame( WeaponSpeed );	//시작 프레임
...
#if __CONTENTS(__FAST_TRANSFORTER || __SECOND_TRANSFORTER)
							// by jin. 
							// 1. 신규 이동수단의 경우, 서몬 아이템들 보다 우선한다. 또한, 신규 이동수단이 두 가지 이상 존재할 경우, 우선순위가 필요하다.
							//	기존의 FindItemAll 함수는 실재 인벤토리 내의 배치 순서가 아니라 최초 서버에서 내려받은 순서에 의해 우선순위가 결정된다.
							//	이는, 유저들의 불편사항이 될 수 있으므로 인벤토리 내의 배치 순서를 기준으로 앞에 배치된 아이템을 찾아 주는 함수를 새로 만들어 사용한다.
							// 2. 신규 이동수단은 서브인벤에 있을 때는 사용이 불가능 하다. 따라서, 서브인벤까지 포함한 배치 순서에 의한 아이템 찾기 함수는 사용하지 못한다.
							//	후에, 서브인벤에 있을 대도 사용이 가능하게 할 때는 사용이 가능할 것이므로 주석 처리하여 으로 남겨둔다. (FindItemAllOrderByIndex)
...
//		// 2004, 6, 9 sobeit add end - 테스트, 흡혈 성공 메세지 온담에 동작하게..-_-
		...
		if (IsStop()) // || m_Action!=ACTION_ATTACK)
		{
			...
			// 내가 Slayer인 경우는 Darkness안에 들어간 캐릭을 쫓아갈 수 없다.
			if (pCreature==NULL 
				// 시체한테 쓸 수 있는 스킬 정보를 추가해야한다. FUCK!
				|| pCreature->IsDead()
...
//		}
		...
		{	
			...
			// 아우스터즈 시체 오른클릭으로 바로 흡영하게 하는 코드 주석 처리 
/*
#ifdef __METROTECH_TEST__
			if (IsOusters())
			{
...
#if __CONTENTS(__FAST_TRANSFORTER||__SECOND_TRANSFORTER)
//슬레이어의 신규이동수단(헬파이어)을 타고 있을 경우 -- moveTimes를 강제로 1줄인다. 즉, 강제로 4프레임자리를 3프레임으로...
...
/*
				...
				if (test_LastSkillType == SkillType)
				{
					...
					switch                           (SkillType)
					{
					case 378: wsprintf (test_SkillName, " 도 : 블레이드콤보(Blade Combo) : 10 ~ 10"); break;
					case 405: wsprintf (test_SkillName, " 도 : 범 크래쉬 워크(Bomb Crash Walk) : 300 ~ 300"); break;
					case 254: wsprintf (test_SkillName, " 도 : 블레이즈 워크(Blaze Walk) : 30 ~ 50"); break;
					case 129: wsprintf (test_SkillName, " 도 : 문라이트 세버(Moonlight Sever) : 8 ~ 8"); break;
					case 27: wsprintf (test_SkillName, " 도 : 토네이도 세버(Tornado Server) : 10 ~ 20"); break;
					case 26: wsprintf (test_SkillName, " 도 : 허리케인 콤보(Hurricane Combo) : 10 ~ 10"); break;
					case 21: wsprintf (test_SkillName, " 도 : 와일드 스매쉬(Wild Smash) : 10 ~ 10"); break;
					case 20: wsprintf (test_SkillName, " 도 : 트리플 브레이크(Triple Break) : 10 ~ 10"); break;
					case 19: wsprintf (test_SkillName, " 도 : 스파이럴 슬레이(Spiral Slay) : 10 ~ 10"); break;
					case 18: wsprintf (test_SkillName, " 도 : 싱글 블로우(Single Blow) : 10 ~ 10"); break;

					case 404: wsprintf (test_SkillName, " 검 : 샤인 스워드(Shine Swork) : 100 ~ 450"); break;
					case 377: wsprintf (test_SkillName, " 검 : 소드 콤보(Sword Combo) : 10 ~ 10"); break;
					case 253: wsprintf (test_SkillName, " 검 : 블리츠 슬라이딩(Blitz Sliding) : 20 ~ 40"); break;
					case 12: wsprintf (test_SkillName, " 검 : 라이트닝 핸드(Lightning Hand) : 4 ~ 4"); break;
					case 14: wsprintf (test_SkillName, " 검 : 소드 웨이브(SWORD Wave) : 10 ~ 10"); break;
					case 13: wsprintf (test_SkillName, " 검 : 스네이크 콤보(Snake Combo) : 10 ~ 10"); break;
					case 7: wsprintf (test_SkillName, " 검 : 레인보우 슬래셔(Rainbow Slasher) : 10 ~ 10"); break;
					case 10: wsprintf (test_SkillName, " 검 : 크로스 카운터(Cross Counter) : 20 ~ 50"); break;
					case 8: wsprintf (test_SkillName, " 검 : 썬더 스파크(Thunder Spark) : 10 ~ 10"); break;
					case 6: wsprintf (test_SkillName, " 검 : 트리플 슬래셔(Triple Slasher) : 10 ~ 10"); break;
					case 5: wsprintf (test_SkillName, " 검 : 더블 임팩트(Double Impact) : 10 ~ 10"); break;

					case 406: wsprintf (test_SkillName, " 총 : 세터라이트 범(Satellite Bomb) : 100 ~ 450"); break;

					case 417: wsprintf (test_SkillName, " 공통 : 가죽 벗기기(Tanning) : 400 ~ 400"); break;

					case 379: wsprintf (test_SkillName, " 뱀파 : 뱀파이어 콤보(Vampire Combo) : 10 ~ 10"); break;
					case 328: wsprintf (test_SkillName, " 뱀파 : 바이어런트 팬텀(Violent Phantom) : 0 ~ 0"); break;
					case 414: wsprintf (test_SkillName, " 뱀파 : 피의 저주(Blood Curse) : 450 ~ 450"); break;
					case 276: wsprintf (test_SkillName, " 뱀파 : 탈론 오브 크로우(Talon OF Crow) : 6 ~ 6"); break;
					case 93: wsprintf (test_SkillName, " 뱀파 : 블러디 네일(Bloody Nail) : 5 ~ 20"); break;
					case 85: wsprintf (test_SkillName, " 뱀파 : 애시드 터치(Acid Touch) : 6 ~ 6"); break;
					case 199: wsprintf (test_SkillName, " 뱀파 : 핸즈 오브 위즈덤(Hands OF Wisdon) : 6 ~ 6"); break;
					case 80: wsprintf (test_SkillName, " 뱀파 : 포이즈너스 핸즈(Poisonous Hands) : 6 ~ 6"); break;

					case 365: wsprintf (test_SkillName, " 아우 : 헤일로(Halo) : 150 ~ 50"); break;
					case 380: wsprintf (test_SkillName, " 아우 : 아우스터즈 콤보(Ousters Combo) : 10 ~ 10"); break;
					case 397: wsprintf (test_SkillName, " 아우 : 휘두르기(Brandish) : 50 ~ 200"); break;
					case 219: wsprintf (test_SkillName, " 아우 : 플러리쉬(Flourish) : 10 ~ 10"); break;
					case 400: wsprintf (test_SkillName, " 아우 : 파이어 웨이브(Fire Wave) : 50 ~ 200"); break;
					case 223: wsprintf (test_SkillName, " 아우 : 백스탭(Back Stab) : 10 ~ 10"); break;
					case 304: wsprintf (test_SkillName, " 아우 : 디스턴스 블리츠(Distance Blitz) : 10 ~ 70"); break;
					case 225: wsprintf (test_SkillName, " 아우 : 감마 촙(Cammna Chop) : 10 ~ 40"); break;
					case 246: wsprintf (test_SkillName, " 아우 : 흡영(Absorb Soul) : 0 ~ 0"); break;
					case 221: wsprintf (test_SkillName, " 아우 : 샤프 라운드(Sharp Round) : 10 ~ 10"); break;
					case 296: wsprintf (test_SkillName, " 아우 : 시프트 브레이크(Shift Break) : 10 ~ 50"); break;
					case 297: wsprintf (test_SkillName, " 아우 : 워터 실드(Water Shield) : 10 ~ 10"); break;
					case 298: wsprintf (test_SkillName, " 아우 : 디스트럭션 스피어(Destruction Spear) : 10 ~ 50"); break;
					case 299: wsprintf (test_SkillName, " 아우 : 블레스 파이어(Bless Fire) : 10 ~ 10"); break;
					case 300: wsprintf (test_SkillName, " 아우 : 페이틀 스니크(Fatal Snick) : 10 ~ 10"); break;
					case 301: wsprintf (test_SkillName, " 아우 : 샌드 크로스(Sand Cross) : 10 ~ 10"); break;
					case 302: wsprintf (test_SkillName, " 아우 : 더킹 월러프(Ducking Wallop) : 20 ~ 100"); break;
					case 303: wsprintf (test_SkillName, " 아우 : 차징 어택(Charging Attack) : 50 ~ 100"); break;
					case 399: wsprintf (test_SkillName, " 아우 : 음파공격(Sound Wave) : 50 ~ 200"); break;
					
					default : wsprintf (test_SkillName, " 미 등록"); break;
					}
					int		StartFrame		= (*g_pActionInfoTable)[m_nUsedActionInfo].GetStartFrame( m_WeaponSpeed );
	...
	if( GetPetID() != OBJECTID_NULL)
	{
		...
		if(pFakeCreature != NULL)
		{
			...
			if(pPetItem != NULL)
			{
				...
				int petDelayTime = 0;
				if(pPetItem->GetItemType() >= 3 && pPetItem->GetItemType() <= 5)// 2 차 펫이면
...
//		&& g_pUserInformation->LogoutTime == 0
		...
		)// && !HasEffectStatus(EFFECTSTATUS_CAUSE_CRITICAL_WOUNDS))	//IsNotDelay())
	{
		...
		*/
		//--------------------------------------------------------
		// 무슨 effect가 걸려있다면 2배 느리게 움직인다. // IsInFuryOfGnome 을 이동 , 공속도 느려지게 만들어야 한다.
		//--------------------------------------------------------
			...
			HasEffectStatus( EFFECTSTATUS_FROZEN_ARMOR_TO_ENEMY ))
		{
			...
			/*|| (IsInFuryOfGnome() && !IsOusters()) // 퓨리오브노움 이속 느려지는 효과 삭제 - 2006.11.21 by chyaya*/
		...
		else if (!m_bFastMove)
		{
			...
			if (IsLockMode() && !IsInDarkness())
			{
				...
				else
				{
					...
					//기술을 쓸때는 멈춰 있는 상태 뭔가 기술을 쓰는 상태 이동하는 상태
					//등등의 상태이고 따라서 다음이 되야 시전이 가능한것이 됨으로 모든 행동은 
					//다음이 되야 실행이 된다. 따라서 자기 자신에게 기술을 
					//룩 모드 일때 자기 자신에게 기술을 쓸때 쓴다고 명시 함으로써 다음 타겟이 되는 적을
					//지정했던 크리처로 돌릴수 있다. 따라서 자살버그도 고쳐 진다.
...
{ 
	...
	*/
	...
	{
		/////////////////////////////////////////////////////////////////////////
		// 2006.11.10 chyaya - 피 변화량 표시! (나중에 정리해야함)
	...
	{
		/////////////////////////////////////////////////////////////////////////
		// 2008.3.11 cheon ill zea - 계급경험치 변화량 표시!
			...
			n == MODIFY_ADVANCEMENT_CLASS_GOAL_EXP)
		{
			...
			if(modifyEXP > 0)
			{
				...
				if(isDomain)
				{
					...
					//각 경험치별로 3가지 종류가 있어서 부등호로 조건을 체크해서 
					//각 도메인 마다 디파인 종류가 3가지이기 때문에 3으로 나눈다.
