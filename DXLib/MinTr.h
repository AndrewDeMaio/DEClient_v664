...
inline void _MinTraceA(LPCSTR p)
{
	...
	if (hWnd)
	{
		...
		cd.cbData = static_cast<int>(strlen(p) + 1);
		cd.lpData = (void*)p;
...
inline void _MinTraceW(LPCWSTR p)
{
	...
	if (hWnd)
	{
		...
		cd.cbData = static_cast<int>((wcslen(p) + 1) * sizeof(wchar_t));
		cd.lpData = (void*)p;
		::SendMessage(hWnd, WM_COPYDATA, 0, (LPARAM)&cd);
	}
}
...
#ifdef UNICODE
	vswprintf_s(buffer, sizeof(buffer), pFormat, args);
	_MinTraceW(buffer);
#else
	vsprintf_s(buffer, sizeof(buffer), pFormat, args);
	_MinTraceA(buffer);
#endif
...
inline void _ClMinTraceA(COLORREF col, LPCSTR p)
{
	...
	{
		char buf[1024];
		sprintf_s(buf, sizeof(buf), "%8d,%s", col, p);
		cd.dwData = MIN_NOTUNI | MIN_COL;
		cd.cbData = static_cast<int>(strlen(buf) + 1);
		cd.lpData = (void*)buf;
		::SendMessage(hWnd, WM_COPYDATA, 0, (LPARAM)&cd);
	}
}
...
inline void _ClMinTraceW(COLORREF col, LPCWSTR p)
{
	...
	if (hWnd)
	{
		...
		swprintf_s(buf, sizeof(buf), _T("%8d,%s"), col, p);
...
inline void _CmdMinTraceA(int nCmd, LPCSTR p)
{
	...
	{
		char buf[1024];
		sprintf_s(buf, sizeof(buf), "%8d,%s", nCmd, p);
