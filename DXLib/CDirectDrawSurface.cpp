...
constexpr unsigned short GAMMA_MASK565_R = 0xF800;
constexpr unsigned short GAMMA_MASK565_G = 0x07E0;
constexpr unsigned short GAMMA_MASK565_B = 0x001F;

constexpr unsigned short GAMMA_MASK555_R = 0x7C00;
constexpr unsigned short GAMMA_MASK555_G = 0x03E0;
constexpr unsigned short GAMMA_MASK555_B = 0x001F;
...
{
	// BackSurface�� ���� 
	// CDirectDraw���� Release�ϱ� ������ Release�ؼ��� �ȵȴ�.
...
//----------------------------------------------------------------------
// BackBuffer�� ����ϴ� Surface�̴�.
// CDirectDraw�� Init���� ������ �� �̿��Ѵ�.
//
// `�̹� �ʱ�ȭ�Ǿ� ������ false�� ��ȯ�Ѵ�.
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
// dwCaps�� DDSCAPS_VIDEOMEMORY(default)��
//          DDSCAPS_SYSTEMMEMORY�� �� �� �ִ�.
//
// system memory�� Lock/Unlock�� �̿��� ��� �������� Blt ���� ������..
// video memory�� �� �ݴ��. (�߿�!)
// 
// Lock�� Blt�� ��� ������ �����ؼ� Surface�� memory ��ġ�� �����ؾ� �Ѵ�.
//
// �̹� �ʱ�ȭ�Ǿ� ������ false�� ��ȯ�Ѵ�.
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
// transparency color�� dwValue�� �����Ѵ�. (�ʼ�!!)
//----------------------------------------------------------------------
...
// Lock() : Surface�� ���� �׸��� ���� ����
//
// lpSurface : surface memory ���� ��ġ
// lPitch    : �� �� �� 
//
// Surface�� ���ؼ� Lock()�� ����� �Ĵ� �ݵ�� Unlock()�� ����� �Ѵ�.
//
// `�����ϸ� false�� ��ȯ�ϴ� ��, �� ���� Unlock()�� ������ ���ƾ� �Ѵ�.
//----------------------------------------------------------------------
...
{
	unsigned short* pDestInt = static_cast<unsigned short*>(pDest);
	for (size_t i = 0; i < len; ++i)
	{
		unsigned short px = pDestInt[i];

		// Extract RGB565 channels
		unsigned short r = (px & GAMMA_MASK565_R) >> 11;  // 5 bits
		unsigned short g = (px & GAMMA_MASK565_G) >> 5;   // 6 bits
		unsigned short b = px & GAMMA_MASK565_B;          // 5 bits

		// Apply gamma scaling (approximate original assembly shift/mul)
		r = (r * p) >> 5;
		g = (g * p) >> 5;
		b = (b * p) >> 5;

		// Mask again to fit in 5/6/5 bits
		r &= 0x1F;
		g &= 0x3F;
		b &= 0x1F;

		// Combine back to RGB565
		pDestInt[i] = (r << 11) | (g << 5) | b;
	}
}
...
void CDirectDrawSurface::Gamma4Pixel555(void* pDest, int len, int p)
{
	...
	for (int i = 0; i < len; ++i)
	{
		...
		// Extract RGB555 channels
		unsigned short r = (px & GAMMA_MASK555_R) >> 10;  // 5 bits
		unsigned short g = (px & GAMMA_MASK555_G) >> 5;   // 5 bits
		unsigned short b = px & GAMMA_MASK555_B;          // 5 bits

		// Apply gamma multiplier (original assembly: mul/shr)
		r = (r * p) >> 5;
		g = (g * p) >> 5;
		b = (b * p) >> 5;

		// Mask to fit 5 bits again
		r &= 0x1F;
		g &= 0x1F;
		b &= 0x1F;

		// Combine back into RGB555
		pDestInt[i] = (r << 10) | (g << 5) | b;
	}
}

//----------------------------------------------------------------------
// Copy : pDDsourceSurface�� rect ������ 
//        pDDTargetSurface�� point ��ġ�� Copy�Ѵ�.
//
// `SetTransparency()�� �������� �����Ѵ�.
// `�׳� blt�Ϸ��� BltNoColorkey()�� ����Ѵ�.
//----------------------------------------------------------------------
void
...
{
	static long long	thisTickTime, oldTickTime;
	static long long	frame, fps;
	...
	sprintf_s(framebuf, framebuf, "FPS = %3d", fps);
	...
	COLORREF fcolor, COLORREF bcolor, bool option)
{
	...
	TextOut(hdc, x, y, str, static_cast<int>(strlen(str)));
