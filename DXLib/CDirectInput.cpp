...
void CDirectInput::OnMouseInput()
{
	...
	while (!bDone)
	{
		...
		enum MouseOffSet
		{
			DIMOFSX = offsetof(DIMOUSESTATE, lX),
			DIMOFSY = offsetof(DIMOUSESTATE, lY),
			DIMOFSZ = offsetof(DIMOUSESTATE, lZ)
		};

		switch (od.dwOfs)
		{
		case DIMOFSX:  // Mouse horizontal motion 
		...
		case DIMOFSY:  // Mouse vertical motion 
		...
		case DIMOFSZ:       // Mouse vertical motion 
