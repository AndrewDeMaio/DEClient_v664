...
template <class Type>
bool CFramePack<Type>::SaveToFile(std::ofstream& packFile, std::ofstream& indexFile)
{
	//--------------------------------------------------
	// Size저장 : 0이라도 개수는 저장한다.
	//--------------------------------------------------
	packFile.write((const char*)&this->m_Size, this->s_SIZEOF_SizeType);
	indexFile.write((const char*)&this->m_Size, this->s_SIZEOF_SizeType);

	// 아무것도 없으면..
	if (this->m_pData == NULL || this->m_Size == 0)
		return false;

	//--------------------------------------------------
	// index file을 생성하기 위한 정보
	//--------------------------------------------------
	long* pIndex = new long[this->m_Size];
	...
	//
	//--------------------------------------------------
	for (TYPE_FRAMEID i = 0; i < this->m_Size; i++)
	{
		// CreatureFrame이 file에 쓰여지는 index를 저장
		pIndex[i] = packFile.tellp();

		// CreatureFrame저장
		this->m_pData[i].SaveToFile(packFile);
	}
