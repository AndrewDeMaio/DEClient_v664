...
bool			IsCreatureActionAttack( MCreature *pCreature )
{
	...
	if( pCreature->IsSlayer() )
	{
		//20090122 승직일 경우 액션 값중에 매직 어택이 인식이 안되 다음과 같이 해주어야 함
		//궁극 적으론 action값이 정상적으로컨버젼 되야 하거나 컨버젼 되지 않는게 맞다면 이게 맞을지도...
...
MCreature::MCreature()
{
	...
	m_ColorPersonalShop	= s_PersnalShopColor[0];	// 디폴트 값으로 네츄럴 칼라를 넣는다.
...
//					false);			// 기술 첨부터 시작한다.
		...
		case EFFECTSTATUS_HYDRO_CONVERGENCE_BOMB:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_HYDRO_CONVERGENCE_BOMB, GetX(), GetY(), 0, GetDirection(), GetID(),
								GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_BLIND:	// 화면을 하얀색으로 블라인드하는 이팩트 설정
			if(g_pTopView != NULL && GetID() == g_pPlayer->GetID())
			{
...
//			delayFrame = 32;
	// 2004, 08, 05 sobeit add end - 성문, 길드 타워 파편튀기
		...
		switch( type )
		{
		// Vampire Renewal 기존 구조 따라가기 ㅠ_ㅠ by chyaya
		...
		case EFFECTSPRITETYPE_RENEWAL_TALON_OF_CROW_MALE_ACTION_1_NORMAL :
			...
			// 여기서 break 하면 안됨
			// 밑에 WeaponSpeed에 따른 구문을 처리해줘야 제대로 나온다.
...
//	if (m_NextChatFadeTime >= g_CurrentTime || ( CurPernalShop() == 1 && m_NextChatFadeTime == 0 ) )
	...
	else
	{
		...
		{
#if __CONTENTS(__TIPOJYU_CASTLE)		//구시온 말풍선 띄우기
...
MCreature::UpdateStatus()
{
	...
	// 아래 상태에서 풀려났을때 피가 한꺼번에 차는것을 방지하기 위해서 리젠타임을 동기화 한다.
	//--------------------------------------------------------
	if (!m_bAlive 
...
//		return ACTION_ATTACK;
	...
	//2005.07.06 Sjheon  Combo 기술처리를 위해 Add
/*	if(IsAdvancementClass() && (*g_pActionInfoTable)[action].IsAttack())
	...
	//2005.07.06 Sjheon  Combo 기술처리를 위해 End 
...
// 2005.07.08 combo Skill USE My Creature  만 적용  Sjheon
...
bool	MCreature::IsShowNameCreature() const
{
	...
	bool bNo		= m_CreatureType == 659 ||								// 대지의 뿔
					  m_CreatureType == 673 ||								// 리젠 존 타워
					  (m_CreatureType >= 726 && m_CreatureType <= 729)	||	// 성문
					  (m_CreatureType > 948 && m_CreatureType <1021)	||	// 메네그로스 제단	
						m_CreatureType	== 1041;							// 티포쥬 성 진입 NPC
	return	bOk && !bNo;
}
...
#endif //__FAST_TRANSFORTER
#if __CONTENTS(__FAST_TRANSFORTER)
void MCreature::SetWingEffect1() // 1차 이동수단
{
...
#if __CONTENTS(__SECOND_TRANSFORTER)
void MCreature::SetWingEffect2() // 2차 이동수단
{
	Race raceType = GetRace();
	int spriteType = EFFECTSPRITETYPE_NULL;
	switch(raceType)
...
#if __CONTENTS(__FAST_TRANSFORTER||__SECOND_TRANSFORTER)
//슬레이어만 .. 오토바이 탑승시.. Addon에 등록된다. 따라서, 이 함수는 슬레이어일 경우에만 유효하다.
bool	MCreature::IsRide_Fast_Transforter()
{
	...
	if (addon.bAddon && (addon.ItemType == 8 //신규 이동수단이 추가되면..여기에 ItemType에 하드코딩을 ...
#if __CONTENTS(__SECOND_TRANSFORTER)
			|| addon.ItemType == 9
#endif //__SECOND_TRANSFORTER
		))
