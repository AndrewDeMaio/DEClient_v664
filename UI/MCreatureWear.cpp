...
#if __CONTENTS(__GEAR_SWAP_CHANGE)	// MCreatureWear::SetAddonItem 장착창 교체 조건
...
#if __CONTENTS(__GEAR_SWAP_CHANGE)	//MCreatureWear::RemoveAddonItem 장착창 해제 조건
	if (m_Action==ACTION_STAND
		|| m_Action==ACTION_MOVE
		|| IsSlayer() && m_Action==ACTION_SLAYER_MOTOR_STAND
		|| IsOusters() && m_Action==ACTION_OUSTERS_FAST_MOVE_STAND
			...
			else
			{
				...
				switch (add)
				{
					...
					case ADDON_TROUSER :
					{
						...
						addon.ColorSet2	= m_ColorBody1;//defaultSkinColor;	// 기본 피부색
						addon.bEffectColor = FALSE;
						addon.EffectColorSet = 0;
					}
					break;
...
MCreatureWear::SetCreatureType(TYPE_CREATURETYPE type)
{
	...
	else
	{
		...
		addon2.ColorSet2	= m_ColorBody1;//defaultSkinColor;	// 기본 피부색
