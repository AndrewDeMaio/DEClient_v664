	...
	*/
	...
	/*
		...
		for(int k = 0; k < CFASize; k++)
		{
				...
				{
					// �ش� Action�� ù��° Direction FRAME_ARRAY�� �����´�.
...
//------------------------------------------------
	...
	/*
		...
		const char* strAction[] = {
				...
				"���ݿ�(SR)",		// ACTION_SLAYER_GUN_SR
				"����(AG)",			// ACTION_SLAYER_GUN_AR
				"����(SG)",			// ACTION_SLAYER_GUN_SG
				"�ڵ�����(SMG)",	// ACTION_SLAYER_GUN_SMG
				...
				"��������̵�",		// ACTION_SLAYER_MOTOR_MOVE
				"�����������",		// ACTION_SLAYER_MOTOR_STAND

				"���ݿ�(SR)SLOW",	// ACTION_SLAYER_GUN_SR_SLOW
				"���ݿ�(SR)FAST",	// ACTION_SLAYER_GUN_SR_FAST
				...
				"�ڵ�����(SMG)SLOW",// ACTION_SLAYER_GUN_SMG_SLOW
				"�ڵ�����(SMG)FAST",// ACTION_SLAYER_GUN_SMG_FAST
...
//----------------------------------------------------------------------
// �Ϻ��� Tile�� ImageObject�� Thread Load�Ѵ�.
//----------------------------------------------------------------------
bool
...
// 	}
// 	
// 	// LargeZoneTileID���� SmallZoneTileID�� �����Ѵ�.
// 	LargeZoneTileID -= SmallZoneTileID;
// 	
// 	// LargeZone���� �����ϴ� TileID���� �����Ѵ�.
	...
	if (m_bFade)// || bEvent)
	{
		...
		if (m_delayFrame)
		{
			...
			if (m_delayPostion & FADE_DELAY_POSTION_FRONT)
			{
				...
				{
					if (m_FadeEnd == 0 && 1 == m_FadeValue) // ���巹 ���ⶫ�� ��ο� ���㿡 ��� ����..^^;
					{
						if (g_CurrentFrame - TempFadeFrame > 16 * m_delayFrame) // �Է°���ŭ ���巹�� 5�ʰ� ����
							m_bFade = false;
					}
					else
					{
...
//		}
	...
	//NPC ���� ĳ���Ͱ� ���� ��� NPC�� Ŭ�� ���� �ʾҴ� �κ��� ĳ���Ͱ� �������� ũ�� �� �� �ֵ��� �켱 ������ NPC���� �־���.	
	for (currentY = sY2; currentY >= sY1; currentY--)
	{
		for (currentX = sX1; currentX <= sX2; currentX++)
		{
					...
					for (int c = 0; c < numCreature; c++, iCreature++)
					{
							...
							IsAbleToActionTarget(pCreature, actionTarget))
						{
								...
								//	&& y > rect.top && y < rect.bottom)
							{
								...
								if (pCreature->IsNPC())	//Ŭ���� ���� NPC�� ������ NPC ������ �Ѱ��ش�.
									return pCreature;
							}
						}
					}
...
MTopView::UpdateImageObject(const POINT& newFirstSector)
{
	...
	if (newFirstSector.y != m_FirstSector.y)
	{
		...
		if (newFirstSector.y - m_FirstSector.y < 0)
		{
			...
			// �ٽ� �츱 ��
			lastY = newFirstSector.y + SECTOR_HEIGHT + 1;	// ���� ȭ���� ������ ��

			// ������ �� �� : lasyY+1 ~ eraseY
			eraseY1 = lastY + 1;
			eraseY2 = m_FirstSector.y + SECTOR_HEIGHT + 1;	// ���� ȭ���� ������ ��			
	...
	if (newFirstSector.x != m_FirstSector.x)
	{
		...
		if (newFirstSector.x - m_FirstSector.x < 0)
		{
			...
			// �ٽ� �츱 ��
			lastX = newFirstSector.x + SECTOR_WIDTH + 1;	// ���� ȭ���� ������ ��

			// ������ �� �� : lasyX+1 ~ eraseX
			eraseX1 = lastX + 1;
			eraseX2 = m_FirstSector.x + SECTOR_WIDTH + 1;	// ���� ȭ���� ������ ��			
...
MTopView::DrawZone(int firstPointX, int firstPointY)
{
	...
	// �̸��� ������ ǥ���ϴ°�?
	//-------------------------------------------------
	BOOL bDrawCreatureMyName = FALSE;	// �� �̸�
	//BOOL bDrawCreatureName	= FALSE;	// �� �̸�
		...
		*/
		...
		if (!g_bLight)
		{
			...
			if ((g_pZone->GetID() != 1006 || !g_pPlayer->IsInSafeSector()) &&
				!g_pZone->IsSurvivalZone() &&	// �����̹� ������� ������ ���. Very Hard -_-;;
				g_pZone->GetID() != 1311 &&			// �ƿ콺���� ������ ������ ���.
				!g_pPlayer->HasEffectStatus(EFFECTSTATUS_GHOST))
			{
				//----------------------------------------------------------------
				//
...
void	MTopView::SetTitleEffectPos(POINT* pPoint, int iIndex)	//����Ʈ ��ġ ����
{
	std::vector<S_TITLE_UI_EFFECT>::iterator it = m_TitleUI_EFFECT_Info.begin();
	for (int i = 0; i < m_TitleUI_EFFECT_Info.size(); i++, it++)
	{
...
//			else
			...
			if (!(pCreature->GetCreatureType() >= 371 && pCreature->GetCreatureType() <= 376 || pCreature->GetCreatureType() >= 560 && pCreature->GetCreatureType() <= 563 || pCreature->GetCreatureType() >= 526 && pCreature->GetCreatureType() <= 549 || pCreature->GetCreatureType() == 672 || pCreature->GetCreatureType() == 673))
			{
				if (pCreature->GetCreatureType() == 482 ||
					...
					pCreature->GetCreatureType() == 819)	// Ʈ���� select�ȵǰ�
				{
					RECT rect;
					rect.left = orPoint.x;
					rect.top = orPoint.y;
	...
	if (pItem->IsDropping())
	{
		...
		// if (bDropping) ���� �ּ��κ��� ����� �Ѵ�..
		//---------------------------------------------------------
// for�� ���� colse���� �����۵��������afpk������ ������ ������ ��ǥ��������  �缳�� ���ִ� �κ��Դϴ�.DJ 2006.12.19
/*
			const int numFrames = 6;
			int nFrame[numFrames] = {3, 0, 1, 2, 3, 0};
	...
	*/
	...
	else if (!pCreature->IsPlayerOnly())
	{
		...
		else
		{
			...
			if (levelGap < -30)
			{
				color = RGB(217, 217, 217);	// ȸ�� - ����
			}
			else if (levelGap < -11)
			{
				color = RGB(6, 216, 86);	// ���λ� - �ణ ����
			}
			else if (levelGap < 10)
			{
				color = RGB(239, 228, 3);	// ����� - ����
...
void//m_bDrawRequest�� ��� �ϱ� ���� �ݵ�� false�� ����� ������ ���࿡�� �׸��� �׷����� �ʴ� ��츦 �����ؼ�...
...
MTopView::DrawCreatureNameOverText(POINT* pPoint, MCreature* pCreature)
{
		...
		)
	{
		...
		*/
		...
		//		if(pCreature->GetClassType() == MCreature::CLASS_PLAYER)	// �÷��̾��� ��쿡��
...
// DrawCreatureName
//----------------------------------------------------------------------
// DrawNameCreatureInfo�� �Ķ���ͷ� ����ϴ� DrawCreatureName()
//----------------------------------------------------------------------
...
// DrawCreatureNameOverText
//----------------------------------------------------------------------
// DrawNameCreatureInfo�� �Ķ���ͷ� ����ϴ� DrawCreatureNameOverText()
//----------------------------------------------------------------------
...
void MTopView::VampireComeFromPortalNonTurn(MCreature* pCreature)
{
	...
	else if (creatureType >= 1089 && creatureType <= 1100)		// ī�� ������, ���� ������ ����
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
#endif //__PET_CATS_UPGRADED
#if __CONTENTS(__UPGRADE_MONSTER_PET)
	else if (creatureType >= 1101 && creatureType <= 1112)		// ī��,����..����
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
