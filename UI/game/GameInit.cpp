...
//----------------------------------------------------------------------
// Title Loading - 2001.8.20 ?????? ??? ?�??? global - -;;
//----------------------------------------------------------------------
...
const int			g_AddonSPKLoadingTimes = 20;	// 20??????? ?????? loading???.
...
DrawTitleLoading()
{
	if (g_bTitleLoading)
	{
		...
		//--------------------------------------------------------
		POINT pointZero = {
			(SURFACE_WIDTH-(*g_pTitleLoadingSPK)[SPRITEID_CI].GetWidth())/2,
			(SURFACE_HEIGHT-(*g_pTitleLoadingSPK)[SPRITEID_CI].GetHeight())/2
		};
...
//		g_pBack->Unlock();
		...
		if (gpC_base!=NULL)
		{
			...
			g_Print(331, 451, "??? ?????????.", pPrintInfo);
			pPrintInfo->text_color	= txColor;
			g_Print(330, 450, "??? ?????????.", pPrintInfo);
...
//			if (!CDirect3D::IsHAL())
			...
			POINT deliberationPoint = { SURFACE_WIDTH - (*g_pTitleLoadingSPK)[SPRITEID_CI_DELIBERATION].GetWidth() - 6, 6 };

			pSurface->Lock();

			if(g_TitleSpriteAlpha == 32)
			{
				pSurface->BltSprite(&pointZero, &(*g_pTitleLoadingSPK)[SPRITEID_CI]);
			...
			{
				pSurface->BltSpriteAlpha( &pointZero, &(*g_pTitleLoadingSPK)[SPRITEID_CI], g_TitleSpriteAlpha);
			}
			pSurface->Unlock();
		}
...
//	else
	{		
		...
		// Must be 16-bit to match the back buffer and sprite renderer expectations.
		// InitOffsurface() without a pixel format would inherit 32-bit desktop format,
		// causing the sprite renderer (which writes WORDs) to compress everything to
		// half width and produce wrong colors.
		g_pLast->InitOffsurface16(
	...
	// Back buffer is always 16-bit (RGB565 or RGB555); query that format directly.
	// Do NOT use Get_BPP() here — it queries the 32-bit primary surface, which
	// would set s_bytes_per_px=4 and corrupt all sprite pixel addressing.
	if (CDirectDraw::Is565())
		InitializeGL(16, 5, 6, 5);
	else
		InitializeGL(16, 5, 5, 5);
...
//	gC_vs_ui.SetVersion(CLIENT_VERSION);
	...
	*/	
	...
	//--------------------------------------------------------
	//g_pTopView->SetSurface(g_pLast);	
	if (g_Mode==MODE_CHANGE_OPTION	// ??? ?????? g_pTopView->Init()?? ?? ???.
		&& !g_pTopView->Init())
	{
		return FALSE;
	}			
...
InitSound()
{
	// --- diagnostic: append-mode log so we can trace where InitSound hangs/faults ---
	#define ISND_LOG(msg) do { \
		FILE* _f = fopen("winmain_step.log", "a"); \
		if (_f) { fputs("[InitSound] ", _f); fputs(msg, _f); fputc('\n', _f); fclose(_f); } \
	} while(0)
	#define ISND_LOGF(fmt, ...) do { \
		FILE* _f = fopen("winmain_step.log", "a"); \
		if (_f) { fprintf(_f, "[InitSound] " fmt "\n", __VA_ARGS__); fclose(_f); } \
	} while(0)
	...
	*/
	...
	if (g_DXSound.IsInit())
	{
		ISND_LOG("g_DXSound already Init, returning TRUE");
	...
	{
		ISND_LOG("calling g_DXSound.Init(g_hWnd)");
		bool dxInitOk = g_DXSound.Init(g_hWnd);
		ISND_LOGF("g_DXSound.Init returned %d", (int)dxInitOk);

		if (dxInitOk)
		{
			// filename?? ????.. Wav file?? Load???.
			//for (int i=0; i<(*g_pSoundTable).GetSize(); i++)
			//{
			//	(*g_pSoundTable)[i].pDSBuffer = g_Sound.LoadWav( (*g_pSoundTable)[i].Filename );
			//}
			ISND_LOGF("g_pUserOption=%p", (void*)g_pUserOption);
			int value = g_pUserOption->VolumeSound;

			LONG volume = value*SOUND_DEGREE + SOUND_MIN;

			ISND_LOGF("SetVolumeLimit(%ld)", (long)volume);
			g_DXSound.SetVolumeLimit( volume );

			ISND_LOG("BGMManager::Init");
			BGMManager::Instance()->Init(g_DXSound.GetDS());
			ISND_LOG("BGMManager::Init returned from Andrew");
		}
		else
		{
			ISND_LOG("g_DXSound.Init FAILED -> returning FALSE");
		...
		// dwAvailPhys is kind of weird
		// If it's over 100 MB
		// we limit it to 10 MB.
		if (ms.dwTotalPhys >= 100 * 1024 * 1024)
		{
			// Temp code
			ISND_LOGF("g_pClientConfig=%p", (void*)g_pClientConfig);
			g_pClientConfig->MAX_SOUNDPART = 100;
		}

		ISND_LOGF("g_pSoundManager=%p g_pSoundTable=%p", (void*)g_pSoundManager, (void*)g_pSoundTable);

		int soundTableSize = g_pSoundTable ? g_pSoundTable->GetSize() : -1;
		int maxSoundPart = g_pClientConfig ? g_pClientConfig->MAX_SOUNDPART : -1;
		ISND_LOGF("calling g_pSoundManager->Init(soundTableSize=%d, MAX_SOUNDPART=%d)",
			soundTableSize, maxSoundPart);
		g_pSoundManager->Init( g_pSoundTable->GetSize(), g_pClientConfig->MAX_SOUNDPART );
		ISND_LOG("g_pSoundManager->Init returned");
		...
		return FALSE;
		*/
		ISND_LOG("returning TRUE from InitSound");
		return TRUE;
	}
	#undef ISND_LOG
	#undef ISND_LOGF
...
InitSocket()
{
	...
	//----------------------------------------------------------------------
	// ??? ?????? ???????.
	// ???? ?????? $VSHOME/bin??, ??? ?????? $VSHOME/conf ?? ??????? ???.?	// command line ???? ??? ?????? ?????? ?? ????? ???.
	//----------------------------------------------------------------------
