...
//		g_pEventManager->RemoveAllEvent();
	...
	switch (event)
	{
			case CDirectInput::LEFTDOWN :
				// 2008. 10. 02 "??????? ?????????" ???????ó?? ????? ?????? ???? ??????. ij-ch
				// Start
#if __CONTENTS(__PREMIUM_GIVE_ITEM_UI) // ???????? ?????? ???????? ????, ???ó???? ?????.
...
//			char szbuffer[512];
//			wsprintf(szbuffer,"CTRL+1~8?? ?????ø? ??? ?? ?????? ???? ?? ??????.CTRL+0 ?? ?????????? ??????.");
...
//		|| gC_vs_ui.IsRunningPersnalShop()
	...
	bool bForceAttack = (m_bForceAttack
		...
		if (bForceAttack
			...
			// 2004, 11, 26, sobeit add start - ???? 140 ??æ ??? - ???????? ??, ?????? ????..???? 
			...
			*/
			...
			if ((a & 0x01)==0)
			{
					...
					&& g_pTempInformation->GetMode()==TempInformation::MODE_NULL)
				{
					...
					// 2005, 1, 18, sobeit add start - ????? action???? ???? ??? ????? ü? ?????.
					...
					IsSkillFromOther = g_pPlayer->HasEffectStatus(EFFECTSTATUS_REQUEST_RESURRECT);
