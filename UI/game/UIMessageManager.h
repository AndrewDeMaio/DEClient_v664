...
#if __CONTENTS(__GEAR_SWAP_CHANGE)		//UIMessageManager		 장착창을 교체 할때 UI의 메시지 함수를 만든다.
		static void Execute_UI_GEAR_CHANGE_SEND_PACKET(int left, int right, void* void_ptr);		
#endif	//__GEAR_SWAP_CHANGE

#if __CONTENTS(__TIPOJYU_CASTLE)		//UIMessageManager		 장착창을 교체 할때 UI의 메시지 함수를 만든다.
...
#if __CONTENTS(__PREMIUM_GIVE_ITEM_UI)	//선택된 프리미엄 아이템 지급 요청
