...
// sz_str문자열중, Width폭에 포함될 수 있는 문자열의 Index (base 0)를 알려준다. 
int g_GetStringIndexByWidth(const char* sz_str, int Width, HFONT hfont)
{
	...
	else
	{
		...
		//이제 Index값은.. Width폭을 넘지 않는 문자열의 마지막 캐릭터의 위치.
		resIndex = Index;
	}
...
int g_PrintColorStr2(int x, int y, const char* sz_str, PrintInfo& pi, COLORREF str_rgb, int LimitWidth)
{
	...
	{
		// 출력할 문자열의 픽셀 길이가, 제한 길이보다 길다. 제한 길이를 넘지 않도록.. 문자열을 자른다..
		int Len = strlen(sz_str);
		...
		while (--Index)
		{
			if (!g_PossibleStringCut(sz_str, Index))
			{
				//자를 수 없는 Index (한글등 2바이트 문자구조에서만 .. 가능)일 경우.. 한번 더 --
				Index--;
			}
			//문자열 길이 비교..
		...
		// 문자열의 처음까지 왔다면.. 그럴리는 거의 없겠지만.. 체크
		if (Index == 0)
			return g_PrintColorStr(x, y, sz_str, pi, str_rgb);
		// 아니라면.. Index 만큼만.. 그린다. 일단.. 좀더 자른 다음에.. '..'을 붙여주자.
...
void g_DrawText(RECT* pRt, const char* sz_str, PrintInfo* p_print_info)
{
	...
	if (sz_str != NULL)
	{
		...
		//sz_str의 가로픽셀 크기와 pRt의 폭을 비교하여, 문자열을 라인피드될 수 있도록 선 처리한다.
