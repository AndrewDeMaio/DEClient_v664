...
C_VS_UI_FRIEND_SYSTEM::C_VS_UI_FRIEND_SYSTEM()
{
	...
	m_p_friend_list_view->SetColumnHeader("이름",0,90);
	m_p_friend_list_view->SetColumnHeader("지역",1,150);
	m_p_friend_list_view->SetColumnHeader("접속상태",2,65);
	m_p_friend_list_view->SetColumnHeader("채널",3,0);
...
void C_VS_UI_FRIEND_SYSTEM::Show()
{
	...
	{// 정보창 옵션
		...
		g_FL2_GetDC();
		// 친구 추가 확인창 팝업
		g_PrintColorStr(str_pos_x,confirm_pos_y,"친구 추가 받기 거부",gpC_base->m_dialog_msg_pi,RGB_BLACK);
		g_FL2_ReleaseDC();
...
C_VS_UI_FRIEND_INFO::C_VS_UI_FRIEND_INFO()
{
	...
	m_friend_info_edit_box.AddString("메모를 작성 하시려면 여기를 클릭해 주세요.");
...
void C_VS_UI_FRIEND_INFO::Show()
{
	...
	{// 친구 정보
		...
		// 이름
		g_PrintColorStr(str_pos_x,str_pos_y,"이     름",m_print_info,RGB_BLACK);
		str_pos_y += 18;
		// 계급
		g_PrintColorStr(str_pos_x,str_pos_y,"계     급",m_print_info,RGB_BLACK);
		str_pos_y += 18;
		// 레벨
		g_PrintColorStr(str_pos_x,str_pos_y,"레     벨",m_print_info,RGB_BLACK);
		str_pos_y += 18;
		// 채널
		g_PrintColorStr(str_pos_x,str_pos_y,"채     널",m_print_info,RGB_BLACK);
		str_pos_y += 18;
		// 위치
		g_PrintColorStr(str_pos_x,str_pos_y,"위     치",m_print_info,RGB_BLACK);
		str_pos_y += 18;
		// 팀,클랜,길드
		if (g_pPlayer && g_pPlayer->IsSlayer())
				g_PrintColorStr(str_pos_x,str_pos_y,"    팀   ",m_print_info,RGB_BLACK);
		else if (g_pPlayer && g_pPlayer->IsVampire())
			g_PrintColorStr(str_pos_x,str_pos_y,"클     랜",m_print_info,RGB_BLACK);
		else if (g_pPlayer && g_pPlayer->IsOusters())
			g_PrintColorStr(str_pos_x,str_pos_y,"길     드",m_print_info,RGB_BLACK);
		str_pos_y += 18;
		// 성향
		g_PrintColorStr(str_pos_x,str_pos_y,"성     향",m_print_info,RGB_BLACK);
		str_pos_y += 18;
		// 기여도
		g_PrintColorStr(str_pos_x,str_pos_y,"기 여 도",m_print_info,RGB_BLACK);
		...
		case RACE_SLAYER:
			{
				...
				if (!m_user_society.empty())
					...
					g_PrintColorStr(info_area_x+3,info_data_y+2,"가입 되어 있지 않습니다."/*(*g_pGameStringTable)[UI_STRING_MESSAGE_NOT_JOIN_ANY_CLAN].GetString()*/, gpC_base->m_chatting_pi, RGB_WHITE);
				info_data_y += 18;
		...
		case RACE_VAMPIRE:
			{
				...
				if (!m_user_society.empty())
					...
					g_PrintColorStr(info_area_x+3,info_data_y+2,"가입 되어 있지 않습니다.", gpC_base->m_chatting_pi, RGB_BRIGHT_GRAY);
				info_data_y += 18;
		...
		case RACE_OUSTERS:
			{
				...
				if (!m_user_society.empty())
					...
					g_PrintColorStr(info_area_x+3,info_data_y+2,"가입 되어 있지 않습니다.", gpC_base->m_chatting_pi, RGB_WHITE);
				info_data_y += 18;
...
bool C_VS_UI_FRIEND_INFO::MouseControl(UINT message, int _x, int _y)
{
	...
	if (m_friend_edit_box.PtInRect(CPoint(_x,_y)))
	{
		if (message == M_LEFTBUTTON_DOWN)
		{
			...
			std::string str = "메모를 작성 하시려면 여기를 클릭해 주세요.";
			char* memo = NULL;
...
C_VS_UI_FRIEND_WAIT::C_VS_UI_FRIEND_WAIT()
{
	...
	m_friend_wait_list_view->SetColumnHeader("이름",0,134);
	m_friend_wait_list_view->SetColumnHeader("수락",1,50);
	m_friend_wait_list_view->SetColumnHeader("거부",2,50);
...
void C_VS_UI_FRIEND_WAIT::Run(id_t id)
{
	...
	{// 추가 버튼
		...
		if (index < 0 || index >= m_friend_wait_list_view->GetItemSize())
			return;
		index += m_friend_wait_list_view->GetBeginItemIndex();
...
void C_VS_UI_FRIEND_SYSTEM_OPTION::Show()
{
	...
	{// 정보창 옵션
		...
		// 친구 추가 확인창 팝업
		g_PrintColorStr(str_pos_x,confirm_pos_y,"친구 추가 확인창 팝업 시키기",m_print_info,RGB_BLACK);
		// 쪽지 설정
		g_PrintColorStr(str_pos_x,paper_pos_y,"쪽지를 받기 잠시 꺼둠",m_print_info,RGB_BLACK);
		// 내 정보 보여주기 수락 여부
		g_PrintColorStr(str_pos_x,my_info_pos_y,"내 정보 주기 잠시 꺼둠",m_print_info,RGB_BLACK);
...
C_VS_UI_FRINEND_ADD_INPUT_PLAYER::C_VS_UI_FRINEND_ADD_INPUT_PLAYER()
{
	...
	m_str_message = "친구 관계를 형성하고 싶은 캐릭터 이름을 등록하세요!";
...
C_VS_UI_FRINEND_ADD_IS_PLAYER::C_VS_UI_FRINEND_ADD_IS_PLAYER(const char* player)
{
	...
	m_str_player = player;
	m_str_message = "현재 선택된 캐릭터에게 친구 요청을 하시겠습니까?";
...
C_VS_UI_FRINEND_DEL::C_VS_UI_FRINEND_DEL(const char* player)
{
	...
	m_str_message = "현재 선택된 캐릭터를 친구에서 삭제하시겠습니까?"; 
...
C_VS_UI_FRINEND_OPTION_CONFIRM::C_VS_UI_FRINEND_OPTION_CONFIRM()
{
	...
	m_str_message = "내용이 변경 되었습니다.저장 하시겠습니까?";
