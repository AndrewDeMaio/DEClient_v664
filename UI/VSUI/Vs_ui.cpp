...
//*20091112 fakaus
//일반 유저가 운영자 메뉴 띄울 수 없도록
//CheckMacScreenMode() 호출 위해
extern bool CheckMacScreenMode(); //"PacketFunction.h"
//*
...
void C_VS_UI::HotKey_Background_Music()		//채팅 명령어 /배경음
{
	if (m_pC_game)
		m_pC_game->HotKey_Background_Music();
}

void C_VS_UI::HotKey_Effect_Music()			//채팅 명령어 /효과음
{
	if (m_pC_game)
		m_pC_game->HotKey_Effect_Music();
}

void C_VS_UI::HotKey_Where()				//채팅 명령어 /어디
{
	if (m_pC_game)
		m_pC_game->HotKey_Where();
}

void C_VS_UI::HotKey_Chat_Help()			//채팅 명령어 /채팅도움
{
	if (m_pC_game)
...
-----------------------------------------------------------------------------*/
void C_VS_UI::AddChatToHistory(char * str, const char * sz_id, CHAT_LINE_CONDITION condition, DWORD color)
{
	if (m_pC_game)
...
bool C_VS_UI::MouseControl(UINT message, int x, int y)
{
	...
	if (ret == false && (message == M_LEFTBUTTON_DOWN || message == M_LB_DOUBLECLICK))
	{
		if (gpC_mouse_pointer->IsPickUpItem())
		{
			...
			if(g_pUserOption->DoNotShowItemBayMessageBox)			//아이템을 버릴때 정말 버리겠냐고 물어본다.
			{
	...
	else
	{
		...
		if(accel !=0 /*&& 
			(
				...
				(accel != ACCEL_CHAT && accel != ACCEL_GUILD_CHAT && accel != ACCEL_ZONE_CHAT && accel != ACCEL_WHISPER && accel != ACCEL_PARTY_CHAT ) // 채팅 단축키가 아니고,
			)*/
		)
		{
...
//-----------------------------------------------------------------------------
// C_VS_UI::GetSlotIndicesItemCanReplace
//
// 아이템이 들어갈수 있는 슬로 번호를 수집한다.
//-----------------------------------------------------------------------------
void C_VS_UI::GetSlotIndicesItemCanReplace(const MItem* pItem, std::vector<int>& slotIndices)
{
	int slot_max = 0;
	...
	for(int i = 0; i < slot_max; i++)
	{
		...
		if (CanReplaceItemInGear((MItem*)pItem, i, p_slot_item))
		{
...
//
//-----------------------------------------------------------------------------
void C_VS_UI::RunTeamRegist(bool member, int reg_fee, int rank, const char *date, const char *team_name, int guild_id)
{
	if (m_pC_game)
...
#if __CONTENTS(__TUNING_ITEM)
void C_VS_UI::RunTuningConfirm1(int value, char *name1, char *name2)	// 튜닝 가능
{
	if (m_pC_game)
		m_pC_game->RunTuningConfirm1(value,name1,name2);
}

void C_VS_UI::RunTuningConfirm2(int value, char *name1, char *name2)	// 튜닝 불가능
{
	if (m_pC_game)
		m_pC_game->RunTuningConfirm2(value,name1,name2);
}

void C_VS_UI::RunConfirmError(char *message) // 불가능
{
	if (m_pC_game)
...
void	C_VS_UI::RunNamingChange(MItem* pItem, const char* szCustom)
{
	if(m_pC_game)
