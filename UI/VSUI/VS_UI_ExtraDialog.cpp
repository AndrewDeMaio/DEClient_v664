...
char * C_VS_UI_EDIT_DIALOG::m_sz_question_msg[1] = {"?? ??????            ???? ??ð??????"};

// ???ó?? by sigi
//char * C_VS_UI_MONEY_DIALOG::m_sz_question_msg[1] = {"???? ????? ????????."};
//char * C_VS_UI_MONEY_DIALOG::m_sz_question_msg_for_storage[1] = {"?????? ????? ????????."};
...
void	C_VS_UI_MONEY_DIALOG::KeyboardControl(UINT message, UINT key, long extra)
{
	...
	case MONEY_CAMPAIGN_HELP:
		...
		break;
	case MONEY_WEDDING_CONTRIBUTION:			// ????? ??? 100,000 ?????? ??´?.
		_MAX_MONEY = 10;
		break;
...
// 
//-----------------------------------------------------------------------------
C_VS_UI_MONEY_DIALOG::C_VS_UI_MONEY_DIALOG(int _x, int _y, int center_x, int center_y, void (*exec_fp)(C_VS_UI_DIALOG*, id_t), WORD dd_button, int digit_count, TYPE_MONEY_DIALOG type) :	// type ??? by sigi
	C_VS_UI_DIALOG(_x, _y, center_x, center_y, exec_fp, dd_button)
{
	/*	// ???ó?? by sigi
...
void	C_VS_UI_ASK_DIALOG::InitString()
{
	...
	(*g_pGameStringTable)[UI_STRING_MESSAGE_ASK_DIALOG_TRADE_OTHER_PLAYER]= "%s ??? ???????????";
	(*g_pGameStringTable)[UI_STRING_MESSAGE_ASK_DIALOG_WAIT_OTHER_PLAYER]= "%s ???? ?????? ???????.";
	(*g_pGameStringTable)[UI_STRING_MESSAGE_ASK_DIALOG_CANCEL]	="?????÷??? Cancel?? ????????.";
	(*g_pGameStringTable)[UI_STRING_MESSAGE_ASK_DIALOG_REQUEST_JOIN]= "%s ???? ????? %s?? ?????? ??û??? ??????.";
	(*g_pGameStringTable)[UI_STRING_MESSAGE_ASK_DIALOG_INVITE]= "%s ???? ????? %s?? ??û???????.";
	(*g_pGameStringTable)[UI_STRING_MESSAGE_CANNOT_FIND_RESULT]= "??? ????? ã?? ?? ???????." ;
	(*g_pGameStringTable)[UI_STRING_MESSAGE_ENCHANT_CONFIRM]= "?? ???????? Enchant??ð??????";
	...
	C_VS_UI_DIALOG(_x, _y, center_x, center_y, exec_fp, dd_button)
{
	...
	m_sz_question_msg_temp[0] = "";
	m_sz_question_msg_temp[1] = "";
	...
	case ASK_ENCHANT:
	{
		...
		case 2:		// Pet Revival
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][2];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case 3:		// Pet Critical Hit
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][3];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case 4:		// Pet Transformation
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][4];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case 5:		// Pet Transformation
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][5];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case 6:		// Reset Pet
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][6];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case 7:		// Enhance Weapon
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][9];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case 8:		// Pet Upgrade
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][8];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case 9:		// Enhance Item
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][10];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;
	...
	case ASK_USE_PET_FOOD:
	{
		m_sz_question_msg_temp[0] = m_sz_question_msg[type][0];
		m_sz_question_msg_temp[1] = m_sz_question_msg[type][1];
		m_sz_question_msg_temp[2] = m_sz_question_msg[type][2];

		SetMessage(m_sz_question_msg_temp, 3, SMO_NOFIT);
	}
	break;
	...
	case ASK_USE_EFFECT_ITEM:
		if (value == 1)
		{
			m_sz_question_msg_temp[0] = m_sz_question_msg[type][1];
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;
		}
		else
	...
	case ASK_THIRD_ENCHANT:
		m_sz_question_msg_temp[0] = m_sz_question_msg[type][value];
		SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
		break;
#if __CONTENTS(__REPAIRSYSTEM_RENEWAL)
	case ASK_REPAIRITEMALL:
		...
		sprintf(szFormatted, m_sz_question_msg[type][0].c_str(), temp);
		m_sz_question_msg_temp[0] = szFormatted;
...
C_VS_UI_ASK_DIALOG::~C_VS_UI_ASK_DIALOG()
{
	/*if (m_sz_question_msg_temp[0] != nullptr)
	{
...
void C_VS_UI_NPC_DIALOG::InitNpcDlg(int sprite_num, const char* pname, bool IsShowItem)
{
	...
	// 2005, 1, 14, sobeit add start - npc ?? ??? ItemDescription ??????????????. - ????? ????...
	else
	{
		if (sprite_num == 0)
		{
	...
	case DID_ITEM:
	{
		...
		if (!p_item->IsEmptyItemOptionList() && !p_item->IsUniqueItem() && !p_item->IsQuestItem())
		{
			for (int i = 0; i < min(2, p_item->GetItemOptionListCount()); i++)
			{
				...
				{
					h_name += strcmp(p_item->GetItemOptionName(i), "<null string>") ? p_item->GetItemOptionName(i) : "";
					h_name += " ";
				}
				if (p_item->GetItemOptionEName(i) != NULL)
				{
					e_name += strcmp(p_item->GetItemOptionEName(i), "<null string>") ? p_item->GetItemOptionEName(i) : "";
					e_name += " ";
				}
			}
		}
...
//			m_rep_string.push_back(temp_string);
		...
		// Damage
		if (p_item->GetMaxDamage() != -1) // damage?? ??°??
		{
...
//			}
			...
			/*if (p_item->GetItemOptionListCount() != 0)
			{
				...
				char pPartName[20];
				strcpy(pPartName, ITEMOPTION_INFO::ITEMOPTION_PARTNAME[optionInfo.Part]); // Option?? ?????? ????? ???
				...
				if (pPartName)
				{
					...
					{
						sprintf(sz_temp, "\t??? : %s +%d", pPartName, PlusPoint-100);
						strcat(sz_temp, "%");
					}
					else
						sprintf(sz_temp, "\t??? : %s +%d", pPartName, PlusPoint);
		...
		if (void_ptr2 != NULL)
		{
			...
			if (p_AddItem->GetItemClass() == ITEM_CLASS_ETHEREAL_CHAIN && p_AddItem->GetItemType() == 0)
			{
...
void C_VS_UI_FILE_DIALOG::Start(const char* type)
{
	...
	// Checks how many types are in type variable
	int nType, i;
	...
	for (i = 0; i < nType; i++)
	{
		size_t p_type_end = p_type.find(';');

		if (p_type_end == std::string::npos)
			p_type_end = p_type.length();

		std::string name = p_type.substr(0, p_type_end);

		m_filter.insert(m_filter.begin() + i, name);

		if (p_type_end >= p_type.length())
			break;

		p_type = p_type.substr(p_type_end + 1);
	...
	if (mp_open_current_directory == NULL)
	{
		...
		// ?? bit?? ?????? ??????? drive buffer?? ?????.
		DWORD index = 1;
		char	find_drive_name = 'a'; // a, b, c, d ... ??????...
		...
		SetDescScrollPos(GetDescSize() * (_y - m_scroll_y) / (m_scroll_h+gpC_global_resource->m_pC_assemble_box_button_spk->GetHeight((gbl_vampire_interface?C_GLOBAL_RESOURCE::AB_VAMPIRE_SCROLL_TAG:C_GLOBAL_RESOURCE::AB_SLAYER_SCROLL_TAG))));
					...
					_y > m_string_y && _y < m_string_y + m_scroll_max * m_string_gap)		// File List
				{
					...
					if (m_select == m_old_select && m_select != -1)			// Double Click Check
					{
						...
						else if (m_mode == MODE_PROFILE_SELECT)				// MODE_PROFILE_SELECT ?? ???????????? ???????. 
						{
							Run(OK_ID);
						}
					...
					//m_stay_openfolder=GetTickCount();			// ????????? ???????? ???? Ÿ???
					Timer(true);
...
void	C_VS_UI_FILE_DIALOG::Show()
{
	...
	// ????? ???????
	dwNow = GetTickCount();
	//if((dwNow-m_stay_openfolder)>=1000)			// 1?? ??? ??ü??
	if (Timer(false))
	{
		std::string title;
		if (STAY_FOLDER == 0)
...
-
  ...
  `GetCurrentDirectory()?? ???? dir?? refresh???.
-----------------------------------------------------------------------------*/
void C_VS_UI_FILE_DIALOG::RefreshFileList(char* sz_dirname)
{
	HANDLE				hFind;
	WIN32_FIND_DATA	fd;
	BOOL					next_ok = true; // FindNextFile(hFind, &fd)?? BOOL?? ???????...
