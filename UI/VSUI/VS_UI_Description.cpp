...
void	_Item_Description_Show(Rect rect, void* void_ptr, long left, long right)
{
	...
	//#ifdef _LIB // Client에서는 3D용으로 사용할 수 있다. -.-
	RECT _rect;

	SetRect(&_rect, rect.x, rect.y, rect.x + rect.w, rect.y + rect.h);
	DrawAlphaBox(&_rect, 0, 0, 0, g_pUserOption->ALPHA_DEPTH);

	//	SetRect(&_rect, rect.x, rect.y, rect.x+rect.w, rect.y+rect.h); // -.- TransparentBlt16()에서 값을 변경함.. 아니 이럴수가!
	...
	//#endif
	//		if (gpC_base->m_p_DDSurface_back->Lock()) // Descriptor common background를 출력한다.
	//		{
	...
	//	gpC_base->m_item_name_pi.text_color = ITEM_NAME_NORMAL_RGB;
		...
		// name이 제일 길 수 있기 때문에 name을 기준으로 정한다.
		...
		)
	{
			...
			&& p_item->GetItemClass() != ITEM_CLASS_CODE_SHEET)
		{
			...
			if (p_item->GetItemOptionListCount() > 1)			// Rare 아이템인 경우
			{
...
//		if(p_item->IsGunItem())
		...
		if (p_item->IsIdentified())
		{
			...
			else
			{
				...
				// critical hit
				if (p_item->GetCriticalHit() != -1)//크리트컬 히트가 있는가?
				{
		...
		if (p_item->IsIdentified())
		{
			...
			if (p_AddItem != NULL)
			{
				// 20080904 장착장에서 add아이템들도 옵션이 보여야 한다. 클래스 구분을 할까 말까 고민 했다. 
				//itemtable에 additem인지 아닌지 구분하는 속성을 추가해 이용하면 더 좋을듯 싶다.
				//add start
			...
			!p_item->IsQuestItem())
		{
			...
			if (p_item->IsIdentified())
			{
				if (p_item->IsGearItem())
				{
					...
					// 2005, 1, 18, sobeit add start - 필요 승직 레벨
					...
					// 2005, 1, 18, sobeit add end - 필요 승직 레벨
				}
	...
	if (pMouseItem != NULL && pMouseItem->GetItemClass() == ITEM_CLASS_EVENT_STAR && p_item->IsGearItem())
	{
			...
			if (pMouseItem->GetItemType() == 6)
			{
				...
				if (pMouseItem->GetItemType() == 7 || pMouseItem->GetItemType() == 15 || pMouseItem->GetItemType() == 78 || pMouseItem->GetItemType() == 79)
				{
						...
						pMouseItem->GetItemType() == 59)
					{
						...
						if (pMouseItem->GetItemType() == 12)					// 블루 버드는 옵션 두개만
						{
							...
							if (pMouseItem->GetItemType() == 16)			// 트랜스 아이템 키트
							{
		...
		//2006.05.24 sjheon End
										...
										if (pMouseItem->GetItemType() == 22) // 옐로우 드롭
										{
											...
											if (pMouseItem->GetItemType() == 48) // 옐로우 드롭
											{
												ITEM_CLASS curitemclass = p_item->GetItemClass();
												if (curitemclass != ITEM_CLASS_CORE_ZAP &&
													curitemclass != ITEM_CLASS_ETHEREAL_CHAIN &&
												...
												if (pMouseItem->GetItemType() == 57 || pMouseItem->GetItemType() == 69)
												{
													...
													else if (pMouseItem->GetItemType() == 58 || pMouseItem->GetItemType() == 68)					//크리스탈 아이템
													{
		...
		p_item->GetItemClass() == ITEM_CLASS_PET_ITEM)
	{
		...
		if ((pMouseItem->GetItemType() < 13 ||
			pMouseItem->GetItemType() >= 16 && pMouseItem->GetItemType() <= 19) &&	// 12까지는 레드 드롭
			pPetItem->GetSilver() == 0 &&		// silver는 AttrLevel이 0인 경우는 속성이 없다고 보고 인첸트 가능
			pPetItem->GetNumber() >= 10)		// 펫 레벨 10이상인경우
			bCanUpgrade = 1;

		if (pMouseItem->GetItemType() == 13 || pMouseItem->GetItemType() == 14)
		{
			if (pPetItem->GetCurrentDurability() == 0)		// 현재 HP가 0일때
				bCanUpgrade = 2;
			else
				bCanUpgrade = 3;
		}

		if (pMouseItem->GetItemType() == 15 && // 15는 레드 버드
			pPetItem->GetNumber() == 49 && pPetItem->GetItemOptionListCount() == 0)
			bCanUpgrade = 1;

		//if(pMouseItem->GetItemType() >= 22 || pMouseItem->GetItemType() <= 24)	// 매지컬 펫 체인져 기능 삭제
...
void	_SkillTree_Description_Show(Rect rect, void* void_ptr, long left, long right)
{
	...
	//	assert(status != MSkillDomain::SKILLSTATUS_NULL);
		...
		// 새로운 skill을 배울 수 있는 상태인지 체크해야함... by sigi
#if __CONTENTS(__SECOND_RARE_SKILL)
	...
	//				g_PrintColorStr(px, py, "\"아직 배울 수 없습니다.\"", gpC_base->m_item_desc_pi, RGB_RED);				
...
void _Item_Description_Calculator(void (*fp_show)(Rect, void*, long, long), int x, int y, void* void_ptr, long left, long right, int z_order)
{
	...
	int line_count = 2;//2; // default: name, weight -> 빠짐

	if (g_pTimeItemManager->IsExist(p_item->GetID()))
		line_count++;

	// 2004, 7, 1, sobeit add start - corezap 관련
	if (pAddItem != NULL)
	{
		// 20080904 장착장에서 add아이템들도 옵션이 보여야 한다.
		// add start
		...
		{
			if (addItemType >= 0 && addItemType <= 3) // 질드레 코어잽
				line_count += 3; // 아이템 자체 옵션 1개, 4개 모았을때 옵션 2개
		}

		if (addItemClass == ITEM_CLASS_CUE_OF_ADAM)
		{
			if (addItemType >= 0 && addItemType <= 3) // 큐 오브 아담
				line_count += 5; // 아이템 자체 옵션 1개, 모았을때 옵션 4개
		}
	}
	// 2004, 7, 1, sobeit add end - corezap 관련
...
//	if (p_item->IsIdentified())
	{
		...
		else if (itemClass == ITEM_CLASS_CUE_OF_ADAM)
		{
			...
			//20080214 diesirace 개인 상점에서 볼때는 줄이 넘어가게 된다.
	...
	if (rect.x + rect.w > g_pUserInformation->iResolution_x)// /2) 2로 왜 나눈건지..2009.10.13 수정 - by neclipse 결혼 기념일
