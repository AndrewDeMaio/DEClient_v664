...
//	Set(0, 0, g_pUserInformation->iResolution_x, g_pUserInformation->iResolution_y );
	...
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(m_wHide_Ousters_Selection_Button_X, m_wHide_Ousters_Selection_Button_Y,	
													...
													OUSTERS_ID, this, HIDE_OUSTERS_SELECTION_BUTTON));	// 아우스터즈 버튼을 가림.
#endif	
...
void	C_VS_UI_NEWCHAR::Init_TitleUIInterface_InfInfo()
{
	...
	bool bHighResolution = g_pUserInformation->iResolution_x > 800;
	// 해상도 늘린 후 UI를 중앙에 오도록 하기 위한 좌표 수정용 변수.
...
// 				ConvY = (g_pUserInformation->iResolution_y - m_image_spk.GetHeight(WINDOW_1024_768)) / 2;	//128
	...
	m_wWorld_List_Show_Max			= (WORD)GetPrivateProfileInt(arrstrAppName, "WORLD_LIST_SHOW_MAX", 0, arrstrFileName); // 확인 후 수정.
	...
	m_wChenel_List_Show_Max			= (WORD)GetPrivateProfileInt(arrstrAppName, "CHENEL_LIST_SHOW_MAX", 0, arrstrFileName);	// 확인 후 수정.
...
void C_VS_UI_SERVER_SELECT::SetServerList(LPSTR *name, int *id, int *status, int size, int default_id, bool *nonpk, WORD *pwSlayer, WORD *pwVampire, WORD *pwOusters)
{
	...
	if(m_bl_group	== true)
	{
		...
		for(int i = 0; i < size; i++)
		{
			...
			if(pwSlayer != NULL && pwSlayer != NULL && pwSlayer != NULL)	//왜 전부 같은 변수지?
			{
...
{
#if __CONTENTS(__SERVER_SELECT_REVEWAL)
	//if(m_pserver_slayernum_barcount != NULL && m_pserver_vampirenum_barcount != NULL && m_pserver_ousters_barcount != NULL)	//3종족.
	if(m_pserver_slayernum_barcount != NULL && m_pserver_vampirenum_barcount != NULL/* && m_pserver_ousters_barcount != NULL*/)		//2종족.
	{
