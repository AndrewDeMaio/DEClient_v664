...
//#include "AutoQA.h"		//by kim
//#include "AutoQAWorks.h"	//by kim
	...
	: COperatorCategory(categoryName, parentWindow, position)
{
	...
	CPoint ptButton(15, 15);
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bShowModifyHP, "HP 변화량 보기"));

	ptButton.y += 25;
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bShowModifyEXP, "EXP 변화량 보기"));

	ptButton.y += 25;
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bShowCreatureInfo, "크리처 oid 보기"));

	ptButton.y += 25;
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bShowImageObjectInfo, "정지 오브젝트 iid, sid, vp 보기"));

	ptButton.y += 25;
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bShowAddEffect, "상태 이팩트 패킷 보기"));

	ptButton.y += 25;
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bShowSkillRange, "맵에 스킬 범위 표시"));

	ptButton.y += 25;
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bShowModifyRANKEXP, "계급경험치 변화량 보기"));
	...
	: COperatorCategory(categoryName, parentWindow, position)
{
	...
	CPoint ptButton(15, 15);
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bHideAttachEffect, "몸에 붙는 이팩트 숨기기"));

	ptButton.y += 25;
	m_pC_button_group->Add(new C_VS_UI_EVENT_CHECK_BUTTON(ptButton,
		-1, this, &g_pOperatorOption->bHideSpeechBalloon, "채팅 말풍선 숨기기"));
}
...
//-----------------------------------------------------------------------------
// COperatorCategoryAutoQA
//
//-----------------------------------------------------------------------------



COperatorCategoryAutoQA::COperatorCategoryAutoQA(LPCTSTR categoryName, Window* parentWindow, const CRect& position)
	: COperatorCategory(categoryName, parentWindow, position)
{
	m_pC_button_group = new ButtonGroup(this);
	m_pC_button_group->Init();
	...
	m_pC_button_group->Add(m_pWorkListBox);
...
COperatorCategoryAutoQA::~COperatorCategoryAutoQA()
{
	DeleteNew(m_pC_button_group);
}

void COperatorCategoryAutoQA::RegisterWorks()
{
	/*by kim
	aqa::Robot::Instance().AddWork(new AQAWork_MoonlightBox);

	for(int i = 36; i <= 51; ++i)
		aqa::Robot::Instance().AddWork(new AQAWork_NormalCase(i));
*/
}

void COperatorCategoryAutoQA::Start()
{
...
void COperatorCategoryAutoQA::Show()
{
	m_pC_button_group->Show();
}
...
//by kim
//void COperatorCategoryAutoQA::AddUnit(const aqa::ReportUnit& unit)
//{
//	/*aqa::*/Report::AddUnit(unit);
//}