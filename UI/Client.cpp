...
void InitResolutionConfig()
{
	...
	try
	{
		...
		std::string strScaler = g_pFileDef->getProperty("FILE_INFO_RESOLUTION");
		ScalerConfig.load(strScaler.c_str());
		int nScaler = 0, nSharpen = 50;
		try { nScaler = ScalerConfig.getPropertyInt("Scaler"); } catch (...) { nScaler = 0; }
		try { nSharpen = ScalerConfig.getPropertyInt("Sharpen"); } catch (...) { nSharpen = 50; }
		CD3D9Present::SetScaler(nScaler, nSharpen);
	}
	catch (...)
	{
		// Key absent: scaler off.
	}
