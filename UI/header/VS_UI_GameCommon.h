...
//
// 지급받지 않은 프리미엄 아이템이 존재할 경우 나타나는 button.
//-----------------------------------------------------------------------------
...
private:
	...
	enum IMAGE_SPK_INDEX
	{
		REQUEST_GUARD_SLAYER,		// - 00 슬레이어 액자
		REQUEST_GUARD_VAMPIRE,		// - 01 벰파이어 액자
		REQUEST_GUARD_OUSTERS,		// - 02 아우스터즈 액자
		PARTY_REQUEST,				// - 03	파티가입신청
		PARTY_INVITE,				// - 04	파티초대
		RESURRECT_SLAYER,			// - 05	슬레이어 부활		- 죽었을때.
		RESURRECT_VAMPIRE,			// - 06	뱀파이어 부활		- 죽었을때.
		RESURRECT_OUSTERS,			// - 07	아우스터즈 부활		- 죽었을때.
		LVU_BUTTON,					// - 08	벰파이어 레벨업
		LVU_BUTTON_OUSTERS,			// - 09	아우스터즈 레벨업
		RESURRECT_SCROLL,			// - 10	부활스크롤			- 죽었을때.
		ELIXIR_SCROLL,				// - 11	엘릭서				- 죽었을때.
		ETERNITY,					// - 12	이스니티			- 죽었을때. (슬레이어용인가?)
		SKILL_FROM_OTHER_SLAYER,	// - 13 슬레이어 살리는 스킬
		SKILL_FROM_OTHER_OUSTERS,	// - 14 아우스터즈 살리는 스킬
		SKILL_FROM_OTHER_VAMPIRE,	// - 15 뱀파이어 살리는 스킬
		GIVE_ITEM01,				// - 16 프리미엄 지급아이템 아이콘 01 (현재 UI가 쓸 이미지)
		GIVE_ITEM02,				// - 17 프리미엄 지급아이템 아이콘 02 (현재 UI가 쓸 이미지)
		GIVE_ITEM_CASE01,			// - 18 프리미엄 지급아이템 아이콘 껍데기 01
		GIVE_ITEM_CASE02,			// - 19 프리미엄 지급아이템 아이콘 껍데기 02
	};
...
//	virtual void HotKey_WindowToggle()=0;	// ctrl + space
...
#if __CONTENTS(__GEAR_SWAP_CHANGE)		//	C_VS_UI_GAME	장착창 교체키에 사용 될 가상 함수 선언
	...
	virtual void HotKey_Minimap()=0;		// 제거
	...
	virtual void HotKey_Background_Music()=0;	// 채팅명령어 /배경음
	virtual void HotKey_Effect_Music()=0;		// 채팅명령어 /효과음
	virtual void HotKey_Chat_Help()=0;			// 채팅명령어 /채팅도움
	virtual void HotKey_Where()=0;				// 채팅명령어 /어디
	...
	virtual void Hotkey_Function(int FKey)	{} // HotKey 처리 함수 
...
enum CHAT_LINE_CONDITION
{
	...
	CLD_UNION,				//5
	CLD_INFO,				//6 정보 메시지.
	CLD_GRADEDISPLAY,		//7 사용자 등급 메세지
	CLD_MASTER,				//8 운영자 메시지
...
//	bool	MouseControlExtra(UINT message, int x, int y);	// by sigi
	...
	bool	IsCannotSendChatInput();		// Chat Message 전송이 불가능한 상태인가?
};
...
protected:
	enum GEAR_SPK_INDEX
	{
		...
		// 여기서부턴 슬레이어 전용 (슬레 전용 상수는 사용하지 않는 것으로 보임 - Gamecommon.cpp에 찾아봐도 안나옴.)
		BUTTON_FIRST,
...
#if __CONTENTS(__GEAR_SWAP_CHANGE)	// C_VS_UI_GEAR 장착 창 버튼 및 ID에 사용될 변수 선언
	DWORD							m_dwSendGearID;							//서버에 보낼 GearID
	DWORD							m_dwAccepGearID;						//서버에서 받은 GearID
	DWORD							m_dwGearChange_ID1_Button_Pos_X;		//자착창을 교체할 버튼의 위치
	DWORD							m_dwGearChange_ID1_Button_Pos_Y;
	DWORD							m_dwGearChange_ID1_Button_Width;		//장착창을 교체할 버튼의 크기
	DWORD							m_dwGearChange_ID1_Button_Height;
	DWORD							m_dwGearChange_ID2_Button_Pos_X;		//자착창을 교체할 버튼의 위치
	DWORD							m_dwGearChange_ID2_Button_Pos_Y;
	DWORD							m_dwGearChange_ID2_Button_Width;		//장착창을 교체할 버튼의 크기
...
#if __CONTENTS(__GEAR_SWAP_CHANGE)			// C_VS_UI_GEAR 장착창 교체 함수 및 버튼 보이기 함수 선언
	void	GearChange();
...
private:
	...
	bool						m_bMiniMapDisableZone;	// 미니맵을 출력하지 않는 맵이다.
...
#if __CONTENTS(__GEAR_SWAP_CHANGE)			//장착착 교체 실행 가상 함수 선언
...
//	void	HotKey_WindowToggle();
	...
	void	HotKey_Background_Music();	// 채팅명령어 /배경음
	void	HotKey_Effect_Music();		// 채팅명령어 /효과음
	void	HotKey_Chat_Help();			// 채팅명령어 /채팅도움
	void	HotKey_ZoneChat();			// 채팅명령어 /z
	void	HotKey_GuildChat();			// 채팅명령어 /g
	void	HotKey_UnionChat();			// 채팅명령어 /u
	void	HotKey_Whisper();			// 채팅명령어 /w
	void	HotKey_Where();				// 채팅명령어 /어디
//	void	HotKey_F9();	
//	void	HotKey_F10();
//	void	HotKey_F11();
//	void	HotKey_F12();
	void	HotKey_PartyChat();			// 채팅명령어 /p
	void	HotKey_NormalChat();		// 채팅명령어 /c
	...
	//2008.09.24 shootkj
	//시간제 아이템과 위치를 교체 하려 할경우 서버에서 cannotadd로 에러를 보내준다.
	//다시 말해 교체 될 수 없다. 근데 클라에서는 교체 시도를 한다.
	//교체가 되지 않도록 위치를 다시 원래 대로 바꿔 줘야 한다.
};
...
private:
	...
	MSkillDomain::SKILL_STEP_LIST	m_advance_skill_id_vec;		// 화면에 표시되는 승직 스킬 아이디를 저장한다.
	MSkillDomain::SKILL_STEP_LIST	m_rare_skill_id_vec;		// 화면에 표시되는 레어 스킬 아이디를 저장한다.

	int						m_rare_skill_max;					// 유저가 배울 수 있는 레어스킬의 최대값
...
//		m_pC_button_group->CancelPushState();
	...
	void	_Show5();	// Grade2
	void	_Show5_ACSkillList();		// 승직 스킬 리스트 출력
	
	void	_Show6();	// Rare Skill	by chyaya
	void	_Show6_RareSkillList();		// 레어 스킬 리스트 출력
...
private:
	...
	enum GUILD_SPK
	{
		...
		INDEX_GUILD_DELEGATION_OF_POWER,			// 권한 이임
...
public:
	C_VS_UI_TEAM_REGIST(bool member, int reg_fee, int rank, const char *team_name = NULL, const char *date = NULL, int guild_id = -1); // team_name은 member인 경우에만
	~C_VS_UI_TEAM_REGIST();
...
public :
	...
	enum RESURRECT_MODE
	{
		...
		RESURRECT_MODE_SKILL_FROM_OTHER,	// 다른 사람이 부활시켜 주려한다.
		RESURRECT_MODE_MAX
	};
