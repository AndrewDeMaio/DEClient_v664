...
#else
//server name by larosel
	void	SetServerList(std::string *name, int *id, int *status, int size, int default_id, bool *nonpk = nullptr)	{
...
//	void	SetPortal(int x, int y);
	...
	void	AddChatToHistory(char * str, const char * sz_id, CHAT_LINE_CONDITION condition=CLD_NORMAL, DWORD color = g_color[3][3]);
...
//	C_VS_UI_SKILL_VIEW * GetSkillViewWindow();
	...
	void	RunFileDialog(C_VS_UI_FILE_DIALOG::MODE Mode, const char *type = nullptr);
...
//	void	RunTutorialExitAccept();
	...
	void	RunTeamRegist(bool member, int reg_fee, int rank = 0, const char *date = NULL, const char *team_name = NULL, int guild_id = -1);
...
//	void	SetChangeablePetNickName(bool Is);
	...
	void	RunNamingChange(MItem* pItem, const char* szCustom);
