...
class CREATURETABLE_INFO {
	public :
//아래 csv변수는 크리처 이름을 csv파일로 뽑아 내기 위한 변수이다. 게임 실행에는 연관이 없으므로 경우에 따라서 제거 해도 괜찬다.
//역시 함수도 마찬가지이다.
...
//		const char*				CSV_TOKEN;
		...
		int						EffectStatus;			// 몬스터 몸에 붙는 이팩트 상태
		int						Level;					// 몬스터 레벨
		bool					bFade;					// 어둡게 나오는 몬스터
		bool					bFadeShadow;			// 그림자가 어둡게 나오는 몬스터
		int						EffectSpriteType;		// 몬스터 몸에 붙는 이팩트 스프라이트
