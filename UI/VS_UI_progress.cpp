...
// 			m_Draw_Num == EVENT_REVENGE_OF_THE_MASTERS_2);		// 피 나오는거 그냥 빼기로 했다.
	...
	do 
	{
		m_Draw_Num = (rand() % (EVENT_MAX*2 - 4));	// 총 로딩 이미지 개수 - 티포쥬 로딩 이미지 개수.
	...
	if(bHighResolution)
	{
		if(g_pUserInformation->iResolution_x <= 1024)
			m_Draw_Num = m_Draw_Num + EVENT_MAX*2*RESOLUTION_1024_768;
		else // if(g_pUserInformation->iResolution_x > 1024)
		{
			if(g_pUserInformation->iResolution_y < 960)
				m_Draw_Num = m_Draw_Num + EVENT_MAX*2*RESOLUTION_1280_720;
			else if(g_pUserInformation->iResolution_y < 1024)
				m_Draw_Num = m_Draw_Num + EVENT_MAX*2*RESOLUTION_1280_960;
			else
				m_Draw_Num = m_Draw_Num + EVENT_MAX*2*RESOLUTION_1280_1024;
		}
	}
