...
#define	MAX_BOMB_NUMBER					20
#define	MAX_MINE_NUMBER					20
#define MAX_EFFECT_ITEM_NUMBER			99
...
#define MAX_VAMPIRE_LEVEL				150
#define MAX_OUSTERS_LEVEL				150
#define MAX_PET_ENCHANT_PILE_NUMBER		99
#define MAX_PET_FOOD_PILE_NUMBER		9
// nanomech 2006.03.08 Item 수정 작업 start
#define MAX_COMMON_QUEST_ITEM_NUMBER	50
	...
	if(GetItemClass() == ITEM_CLASS_PET_ITEM)
	{
		...
		if(GetItemColorSet() == 0xFFFF)
		{
			if(GetSilver() > 0)	// 속성이 있는 경우는 색 표시
			{
				...
				ITEMOPTION_TABLE::ITEMOPTION_PART optionPart = static_cast<ITEMOPTION_TABLE::ITEMOPTION_PART>(GetEnchantLevel());
				SetItemColorSet(ITEM_COLOR_PART[optionPart]);
			}
			else
			{
...
void	MMixingItem::UseInventory(TYPE_OBJECTID SubInventoryItemID)
{
	...
	if( itemType >= 0 && itemType <= 8		// A ~ C형
		...
		|| (itemType >= 31 && itemType <= 33) // 다크온라인용 새로 추가되는 믹싱포지
		...
		)
	{
		...
		else
		{
			...
			C_VS_UI_MIXING_FORGE::FORGE_TYPE  ft;	// 이건 사용하고 있지 않음.
			...
			else if(itemType == 21 || itemType == 33)
			{
				fc = C_VS_UI_MIXING_FORGE::CLASS_ACCESSORY;
				ft = C_VS_UI_MIXING_FORGE::TYPE_D;
			}
			else if(itemType == 26 || itemType == 31)
			{
				fc = C_VS_UI_MIXING_FORGE::CLASS_WEAPON;
				ft = C_VS_UI_MIXING_FORGE::TYPE_E;
			}
			else if(itemType == 27 || itemType == 32)
			{
				fc = C_VS_UI_MIXING_FORGE::CLASS_ARMOR;
				ft = C_VS_UI_MIXING_FORGE::TYPE_E;
			}
...
void MEffectItem::UseInventory(TYPE_OBJECTID SubInventoryItemID)
{
	...
	int itemType = GetItemType();
	bool useItem = false;
/*	2009.10.29 스크롤 아이템류 전부 중복 사용 가능, 조건체크 필요없음.
	...
	// 2009.11.03 이펙트 아이템류 전부 분리가 가능하도록 처리 되었으므로 전부 필요 없음.
...
// 	|| itemType == 46 || itemType == 47 || (itemType >= 77 && itemType <= 82)) // 46은 바이퍼 블루...47은 경험의 돌..
// 	{
// 		if (GetNumber() > 1)
// 		{
// 			// 아이템 분리
...
// 		
// 			//----------------------------------------------------
// 			// Inventory에서 item을 몇개0 드는 걸 검증받는다.
// 			// 일단은 1개만 들리도록 한다.
// 			//----------------------------------------------------
// 			g_pPlayer->SetItemCheckBuffer(this, MPlayer::ITEM_CHECK_BUFFER_PICKUP_SOME_FROM_INVENTORY);
// 			return;
...
std::string MPetItem::GetPetName()
{
	...
	case 16:
		petName += (*g_pCreatureTable)[1089].Name.GetString();
		break;
	case 17:
		petName += (*g_pCreatureTable)[1090].Name.GetString();
		break;
	case 18:
		petName += (*g_pCreatureTable)[1091].Name.GetString();
		break;
	case 19:
		petName += (*g_pCreatureTable)[1092].Name.GetString();
		break;
	case 20:
		petName += (*g_pCreatureTable)[1093].Name.GetString();
		break;
	case 21:
		petName += (*g_pCreatureTable)[1094].Name.GetString();
		break;
	case 22:
		petName += (*g_pCreatureTable)[1095].Name.GetString();
		break;
	case 23:
		petName += (*g_pCreatureTable)[1096].Name.GetString();
		break;
	case 24:
		petName += (*g_pCreatureTable)[1097].Name.GetString();
		break;
	case 25:
		petName += (*g_pCreatureTable)[1098].Name.GetString();
		break;
	case 26:
		petName += (*g_pCreatureTable)[1099].Name.GetString();
		break;
	case 27:
		petName += (*g_pCreatureTable)[1100].Name.GetString();
		break;
#endif //__PET_CATS_UPGRADED
#if __CONTENTS(__UPGRADE_MONSTER_PET)
	case 28:
		petName += (*g_pCreatureTable)[1101].Name.GetString();
		break;
	case 29:
		petName += (*g_pCreatureTable)[1102].Name.GetString();
		break;
	case 30:
		petName += (*g_pCreatureTable)[1103].Name.GetString();
		break;
	case 31:
		petName += (*g_pCreatureTable)[1104].Name.GetString();
		break;
	case 32:
		petName += (*g_pCreatureTable)[1105].Name.GetString();
		break;
	case 33:
		petName += (*g_pCreatureTable)[1106].Name.GetString();
		break;
	case 34:
		petName += (*g_pCreatureTable)[1107].Name.GetString();
		break;
	case 35:
		petName += (*g_pCreatureTable)[1108].Name.GetString();
		break;
	case 36:
		petName += (*g_pCreatureTable)[1109].Name.GetString();
		break;
	case 37:
		petName += (*g_pCreatureTable)[1110].Name.GetString();
		break;
	case 38:
		petName += (*g_pCreatureTable)[1111].Name.GetString();
		break;
	case 39:
		petName += (*g_pCreatureTable)[1112].Name.GetString();
		break;
	case 40:
		petName += (*g_pCreatureTable)[1113].Name.GetString();
		break;
