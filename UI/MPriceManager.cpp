		...
		else if ( pItem->GetItemClass() == ITEM_CLASS_SLAYER_PORTAL_ITEM || pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_PORTAL_ITEM )
		{
			...
			if(pItem->GetItemClass()==ITEM_CLASS_SLAYER_PORTAL_ITEM)
			{
				if(pItem->GetItemType() < 3)
					ChargePrice = pow(domainLevel , 1.25)  * 100 ;   // 아이템 가격 계산 (Sjheon 2005.06.02) 
				else if(pItem->GetItemType() == 3 || pItem->GetItemType()  == 4)
					ChargePrice = pow(domainLevel , 1.50)  * 100 ;   // 아이템 가격 계산 (Sjheon 2005.06.02) 
			}
			else
				ChargePrice = pow(domainLevel , 1.3)  * 100 ;   // 아이템 가격 계산 (Sjheon 2005.06.02)
		}
