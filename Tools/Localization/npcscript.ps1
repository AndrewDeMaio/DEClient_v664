...
# mysql.exe: honour $env:DE_MYSQL, else PATH, else the usual install locations.
$script:MYSQL = $env:DE_MYSQL
if(-not $script:MYSQL){
    $onPath = Get-Command mysql.exe -ErrorAction SilentlyContinue
    if($onPath){ $script:MYSQL = $onPath.Source }
}
if(-not $script:MYSQL){
    foreach($c in @(
        "$env:ProgramFiles\MySQL\MySQL Workbench 8.0 CE\mysql.exe",
        "$env:ProgramFiles\MySQL\MySQL Server 8.0\bin\mysql.exe",
        "${env:ProgramFiles(x86)}\MySQL\MySQL Workbench 8.0 CE\mysql.exe")){
        if(Test-Path $c){ $script:MYSQL = $c; break }
    }
}
if(-not $script:MYSQL){ throw "mysql.exe not found. Set `$env:DE_MYSQL to its full path." }

# The server repo sits beside the client repo; override with $env:DE_SERVER_ROOT.
$script:SERVERROOT = $env:DE_SERVER_ROOT
if(-not $script:SERVERROOT){
    $script:SERVERROOT = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..\DEServer_v664')).Path
}

function Invoke-DE([string]$sql){
    $conf=Get-Content (Join-Path $script:SERVERROOT 'conf\gameserver.local.conf') -Encoding UTF8
