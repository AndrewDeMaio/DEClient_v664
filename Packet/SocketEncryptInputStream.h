...
public :
    ...
    uint readEncrypt (uchar  &buf) { uint re = read((char*)&buf, szuchar ); buf = m_Encrypter.convert(buf); return re; }
    uint readEncrypt (short  &buf) { uint re = read((char*)&buf, szshort ); buf = m_Encrypter.convert(buf); return re; }
    uint readEncrypt (ushort &buf) { uint re = read((char*)&buf, szushort); buf = m_Encrypter.convert(buf); return re; }
    uint readEncrypt (int    &buf) { uint re = read((char*)&buf, szint   ); buf = m_Encrypter.convert(buf); return re; }
    uint readEncrypt (uint   &buf) { uint re = read((char*)&buf, szuint  ); buf = m_Encrypter.convert(buf); return re; }
    uint readEncrypt (long   &buf) { uint re = read((char*)&buf, szlong  ); buf = m_Encrypter.convert(buf); return re; }
    uint readEncrypt (ulong  &buf) { uint re = read((char*)&buf, szulong ); buf = m_Encrypter.convert(buf); return re; }
