...
VirtualTempFileManager::~VirtualTempFileManager()
{
	...
	for(; pos != m_TempFileNameSet.end(); ++pos)
	{
		...
		int cnt = m_TempFileNameSet.size();
		char szBuf[128];
		sprintf_s(szBuf, sizeof(szBuf), "Delete Temp File: szTempName = %s, Size = %d\n",
			pos->c_str(), --cnt);
...
#ifdef _DEBUG
			char szBuf[128];
			sprintf_s(szBuf, sizeof(szBuf), "Create Temp File: szTempName = %s, Size = %d\n",
