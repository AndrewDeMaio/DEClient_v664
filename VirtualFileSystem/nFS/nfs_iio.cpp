...
#if defined(_WIN32) && !defined(CYGWIN) && !defined(CYGNUS)
...
static void instrument_init() {
    ...
    ok = 0;
    while (!ok) {
        sprintf_s(output_file_name, sizeof(output_file_name), "nfs_iio.instrument.%d.log%c", i++, 0);
...
static int cache_page_get_sync(nfs_iio_Cache_Page* c) {
    ...
    return reinterpret_cast<uintptr_t>(c->buffer) & nfs_iio_CC_SYNC_MASK;
}
...
static void cache_page_set_sync(nfs_iio_Cache_Page* c, int s) {
    ...
    c->buffer = reinterpret_cast<char*>(reinterpret_cast<uintptr_t>(c->buffer) & nfs_iio_CC_BUF_MASK);
    c->buffer = reinterpret_cast<char*>(reinterpret_cast<uintptr_t>(c->buffer) | (s & nfs_iio_CC_SYNC_MASK));
}
...
static char* cache_page_get_buffer(nfs_iio_Cache_Page* c) {
    ...
    return reinterpret_cast<char*>(reinterpret_cast<uintptr_t>(c->buffer) & nfs_iio_CC_BUF_MASK);
}
...
static int cache_expand(nfs_iio_File* file, int channel, int page) {
    ...
    if (!file->chn[channel]->cache) {
        ...
        file->chn[channel]->cache->pages = nullptr;
    ...
    for (i = oldpgs; i < pgs; i++)
        file->chn[channel]->cache->pages[i] = nullptr;
...
static int cache_page_refresh(nfs_iio_File* file, int channel, int pos) {
    ...
    if (!file->chn[channel]->cache->pages[page])
...
static int cache_destroy(nfs_iio_Cache* c)
{
    ...
    /* free pages */
    for (int i = 0; i < c->nr_pages; i++) 
    {
        if (c->pages[i]) {
            if (c->pages[i]->buffer) {
                auto addr = reinterpret_cast<uintptr_t>(c->pages[i]->buffer);
                addr &= ~uintptr_t(1);
                free(reinterpret_cast<char*>(addr));
            }
        }
    }
...
#if defined(WIN32) && !defined(CYGWIN) && !defined(CYGNUS)
        _chsize(_fileno(file->file), header_size(file) + ns * nfs_iio_BLOCK_SIZEv);
#else
...
nfs_iio_File* nfs_iio_create(char* filename) {
    ...
    strcpy_s(handle->filename, strlen(filename) + 8, filename);
    fopen_s(&handle->file, filename, "w+b");
    if (handle->file == NULL)
...
void nfs_iio_destroy(nfs_iio_File* file) {
    ...
    if (tempfn)
        strcpy_s(tempfn, strlen(file->filename) + 8, file->filename);
    /* close the file */
    nfs_iio_close(file);
    /* remove the file! */
#ifdef WIN32
...
nfs_iio_File* nfs_iio_open(char* filename) {
    ...
    if (nfs_iio_IOMODE & 2) /* write */
        fopen_s(&handle->file, filename, "r+b");
    else
        fopen_s(&handle->file, filename, "rb");
