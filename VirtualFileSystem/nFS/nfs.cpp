...
int nfs_exists(char* filename) {
    ...
    strcpy_s(iio_name, nfs_NAME_LEN, filename);
    strcat_s(iio_name, nfs_NAME_LEN, nfs_IIO_SUFFIX);
    strcpy_s(data_name, nfs_NAME_LEN, filename);
    strcat_s(data_name, nfs_NAME_LEN, nfs_DATA_SUFFIX);
...
static int lock_create(char* filename, int mode) {
    ...
    /* open lock */
    strcpy_s(lock_name, nfs_NAME_LEN, filename);
    strcat_s(lock_name, nfs_NAME_LEN, nfs_LOCK_SUFFIX);
    fopen_s(&f, lock_name, "wb");
    if (f == NULL)
...
nfs_Handle* nfs_start(char* filename, int flags) {
    ...
    strcpy_s(fs->filename, 128, filename);
...
static int __nfs_glob_cbk(nfs_dt_DT*, char* filename, int, void* t) {
    ...
    strcpy_s(tmp->pglob->gl_pathv[n-1], strlen(filename) + 4, filename);
