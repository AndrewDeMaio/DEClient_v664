...
static int fnode_allocate(nfs_dt_DT* dt, char* key) {
    ...
    for (i = 0; i < nr_keynodes; i++) {
        ...
        /* partial key */
        memcpy(kn.name, (char*)key + i * nfs_dt_MAX_SKN_SIZE, nfs_dt_MAX_SKN_SIZE);
        /* write keynode */
...
int nfs_dt_filename_glob(nfs_dt_DT* dt,
    ...
    start = p_lookup_key_n(dt, prefix, static_cast<int>(strlen(prefix)) * 8);
