...
nfs_data_Data* nfs_data_create(char* filename) {
    ...
    if (!data->filename)
        return NULL;
    strcpy_s(data->filename, strlen(filename) + 8, filename);
    fopen_s(&data->file, filename, "w+b");
    if (data->file == NULL)
...
nfs_data_Data* nfs_data_open(char* filename) {
    ...
    if (nfs_data_IOMODE & 2) /* write */
        fopen_s(&data->file, filename, "r+b");
    else
        fopen_s(&data->file, filename, "rb");
...
void nfs_data_destroy(nfs_data_Data* data) {
    ...
    if (tempfn)
        strcpy_s(tempfn, strlen(data->filename) + 8, data->filename);
    /* close and destroy */
    nfs_data_close(data);
    /* remove file */
#ifdef WIN32
