...
bool VirtualFileSystem::Start(LPCTSTR filename, int flags) 
{
	...
	{
		// �ش��̸� dpk������ �ִ��� üũ
		strcpy_s(szVFile, MAX_PATH, filename);
		strcat_s(szVFile, MAX_PATH, nfs_DATA_SUFFIX);
		if(_access(szVFile, 0)) return false;
	}

	// ������ ������������ ����� ��� dpl ������ ���� �ȴ�.
	strcpy_s(szVFile, MAX_PATH, filename);
	strcat_s(szVFile, MAX_PATH, nfs_LOCK_SUFFIX);
	remove(szVFile);
...
void VirtualFileSystem::AddFile(LPCTSTR srcname, bool compress)
{
	...
	strcpy_s(szUniquePath, MAX_PATH, srcname);
...
bool VirtualFileSystem::IsFileExist(LPCTSTR filename) const
{
	   ...
	   dwFileAttri & FILE_ATTRIBUTE_DIRECTORY)
	{
		if(m_NFS.is_open())
		{
			...
			strcpy_s(szUniqueName, MAX_PATH, filename);
...
bool VirtualFileSystem::CreateRealFile(LPCTSTR srcname, LPCTSTR destname)
{
	...
	strcpy_s(szUniqueBuf, MAX_PATH, srcname);
...
#endif
	//by viva
	int size = strlen(srcname);
	bool bXor = false;
	if( size >=22 )
	    if(strncmp(&srcname[size-22], "vampire_bat_spear2.wav", 22) == 0) 
	        bXor = true;
	*/
	char	buffer[FILE_COPY_BUFFER_SIZE];
	int		readCount = 0;
...
//#endif
	...
	for(;;)
	{
		...
		if(readCount == 0) break;
//		if(bXor) for(int i=0; i< readCount; i++) buffer[i]^=0xFF;
