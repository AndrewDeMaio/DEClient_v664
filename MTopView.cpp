...
//					}
	...
	if (pItem->IsDropping())
	{
		...
		//---------------------------------------------------------
// for문 부터 colse까지 아이템드롭프레임afpk파일의 프레임 순서와 좌표조정값을  재설정 해주는 부분입니다.DJ 2006.12.19
/*
			const int numFrames = 6;
			int nFrame[numFrames] = {3, 0, 1, 2, 3, 0};
...
void MTopView::VampireComeFromPortalNonTurn(MCreature* pCreature)
{
	...
	else if(creatureType >= 1083 && creatureType <= 1085)		// 고양이
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
#endif //__PET_CATS
#if __CONTENTS(__PET_CATS_UPGRADED)
	else if(creatureType >= 1089 && creatureType <= 1100)		// 카라 고양이, 델라 고양이 전부
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
#endif //__PET_CATS_UPGRADED
#if __CONTENTS(__UPGRADE_MONSTER_PET)
	else if(creatureType >= 1101 && creatureType <= 1112)		// 카라,델라..전부
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
