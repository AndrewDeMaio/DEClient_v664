...
//----------------------------------------------------------------------
bool CSpritePackList::SaveToFile(std::ofstream& spkFile, std::ofstream& indexFile)
{
	// 초기화되지 않았으면 
	if (m_listSprite.empty())
		return false;
	...
	//--------------------------------------------------
	long long* pIndex = new long long[size];
	...
	while (iSprite != m_listSprite.end())
	{
		...
		auto pos = spkFile.tellp();
		auto offset = pos - std::streampos(0);

		pIndex[i++] = static_cast<long long>(offset);
	...
	for (i = 0; i < size; i++)
	{
		indexFile.write(reinterpret_cast<const char*>(&pIndex[i - 1]), sizeof(long long));
	}
